  ...
  beforeEach(done => {
    ...
    lib.dbHelpers.dropAll()
      ...
      .then((res) => {
        ...
        return lib.dbHelpers.addAndLoginCustomer('test@test', "123456", {
          addresses: [address]
        })
      })
      .then(res => {
...
describe("GET Tickets Order By RecieverId", () => {
  ...
  it('expect all ticket for each orderline related to reciver_id', function (done) {
    this.done = done;
    ...
    rp({
      ...
      .then(res => {
        expect(res.statusCode).toBe(200);
        expect(res.body[0]._id).toEqual(orderId.toString());
        expect(res.body[0].tickets.length).toBe(1);
        done();
      })
      .catch(lib.helpers.errorHandler.bind(this));
  });
  ...
  it('expect error user dont access to this url', function (done) {
    ...
    rp({
      ...
      .then(res => {
        this.fail('expect error when reciver_id is not valid');
        done();
      })
      .catch(err => {
