...
describe('POST Search ScanInternalDeliveryBox', () => {
  ...
  it('should get order line which is not in hub and is not c&c (palladium)' , async function (done) {
    try {
      ...
      for (let i = 0; i < 3; i++) { // add 2 order line of first product instance for order 2
        orders[1].order_lines.push({
          paid_price: 0,
          product_id: products[0].id,
          product_instance_id: products[0].instances[0].id,
      ...
      for (let i = 0; i < 3; i++) { // add 2 order line of second product instance for order 2
        orders[1].order_lines.push({
          paid_price: 0,
          product_id: products[0].id,
          product_instance_id: products[0].instances[1].id,
  ...
  // ScanInternalReturnBox
  xit('should get order line which is ScanInternalReturnBox ', async function (done) {
    try {
      this.done = done;
