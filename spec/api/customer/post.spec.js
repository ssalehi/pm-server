...
describe('Guest User', () => {
  beforeEach(done => {
    lib.dbHelpers.dropAll()
      .then(() => {
        return lib.dbHelpers.addCustomer('saman@gmail.com', '1213', {
          is_verified: false,
          is_guest: true,
          first_name: 'Saman',
          surname: 'Vaziri',
  ...
  xit("should get error when email exist", function (done) {
    this.done = done;
  ...
  xit('should add new guest', function (done) {
    ...
    rp({
      ...
      expect(res.length).toBe(1);
      expect(res[0].username).toBe('saman@gmail.com');
      expect(res[0].first_name).toBe('saman');
      expect(res[0].surname).toBe('vaziri');
      expect(res[0].mobile_no).toBe('1234567890');
      expect(res[0].gender).toBe('m');
      expect(res[0].is_verified).toBe(false);
      done();
    }).catch(lib.helpers.errorHandler.bind(this));
  })
});