...
class DSS extends Ticket {
  ...
  async afterInboxScan(order, product, user) {
    try {
      ...
      if ([_const.ORDER_LINE_STATUS.default,
      _const.ORDER_LINE_STATUS.WaitForOnlineWarehouse,
      _const.ORDER_LINE_STATUS.Renew
  ...
  async afterCancel(order, orderLine, user) {
    try {
      ...
      if (!orderLine) { // order cancellation
      } else {

      }

      let cancelOrderLine = async (_order, _orderLine, user) => {
        let lastTicket = order.tickets[order.tickets.length - 1];
