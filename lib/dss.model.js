  ...
  async afterSoldOut(order, orderLine, userId, warehouseId) {
    try {
      ...
      if (!order.is_collect || (order.is_collect && order.address.warehouse_id.toString() !== warehouseId.toString())) {
         ...
         *   */ 
        await super.setOrderLineAsReadyToDeliver(order, orderLine, userId, warehouseId); 
    ...
    try {
      ...
      if (!isReturn) {
        if (isCC || (!isCC && isExternal)) {
          await super.setOrderLineAsChecked(order, foundOrderLine, user.id, user.warehouse_id);
