...
class DSS extends Ticket {
  ...
  async afterFinalCheck(order, product, user, isExternal = false, isCC = false, isReturn = false) {
    try {
      ...
      let findOrderLine = (isCancel = false) => {
        return order.order_lines.find(x => {
      ...
      let canceledOrderLine = findOrderLine(true);
      if (canceledOrderLine) {
        if (user.warehouse_id.toString() !== hub._id.toString()) {
          return super.setOrderLineAsVerfiedByOnlineWarhouse(order, canceledOrderLine, user.id, user.warehouse_id);
        } else {
          return super.setOrderLineAsDelivered(order, canceledOrderLine, user.id, hub._id);
        }

      }

      let foundOrderLine = findOrderLine();
      if (!foundOrderLine)
        throw error.orderLineNotFound;
