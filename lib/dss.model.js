...
class DSS extends Ticket {
    ...
    try {
      ...
      if (user.warehouse_id !== hub._id.toString()) // scan in central warehouse or other shops
      {
        ...
        if (isReturn) {
          ...
          await this.setOrderLineReturnType(order, orderLine, user, _const.RECEIVE_TYPE.Return)
        }
  ...
  async setOrderLineReturnType(order, orderLine, user, returnType) {
    await super.setOrderLineAsWatingForReturn(order, orderLine, user.id, user.warehouse_id);
    return new Offline(this.test).requestForReturn(
      order,
      orderLine,
      productInstance.barcode,
      user,
  ...
  async afterReturnVerified(order, orderLine, userId, hubId) {
    try {
      ...
      let hasDamageWithRefund = foundOrderLine.tickets.map(x => x.status).includes(_const.ORDER_LINE_STATUS.DamageWithRefund);
      let hasDamageWithoutRefund = foundOrderLine.tickets.map(x => x.status).includes(_const.ORDER_LINE_STATUS.DamageWithoutRefund);
  ...
  async afterDamageInformed(order, orderLine, user, type) {
    try {
      ...
      await this.setOrderLineReturnType(order, orderLine, user, type);
