...
class DSS {
  ...
  afterInvoiceVerification(orderId, invoiceNo, userId, warehouseId) {
    ...
    return new WarehouseModel(this.test).getAll()
      ...
      .then(res => {
        Promise.all(foundOrder.order_line.map(orderLine => new TicketModel(DSS.test).setTicket(foundOrder, orderLine, _const.ORDER_STATUS.InvoiceVerified, userId, warehouseId)))
      })
      .then(res => {
        if (foundOrder.is_collect) {
          Promise.all(foundOrder.order_line.map(orderLine => new TicketModel(DSS.test).setTicket(foundOrder, orderLine, _const.ORDER_STATUS.Delivered, userId, warehouseId, null, true)))
        } else {
          Promise.all(foundOrder.order_line.map(orderLine => new TicketModel(DSS.test).setTicket(foundOrder, orderLine, _const.ORDER_STATUS.ReadyToDeliver, userId, warehouseId)))
        }
      })
      .then(res => {
        socket.sendToNS(warehouseId);
