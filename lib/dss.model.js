...
class DSS extends Ticket {
  ...
  async afterRequestForPackage(delivery, userId) {
    try {
      ...
      if (delivery.to.customer) { // external delivery
        ...
        let orderIds = [];
        let orderLineIds = [];
        let orderLineIds = delivery.order_details.forEach(x => {
  ...
  async afterFinalCheck(order, product, user) {
    try {
      ...
      if (!order.is_collect) {
        ...
        await new DeliveryModel(this.test).initiate(foundOrder, foundOrderLine,
          {warehouse_id: mongoose.Types.ObjectId(user.warehouse_id)},
          {warehouse_id: hub._id});

        await super.setOrderLineAsReadyToDeliver(foundOrder, foundOrderLine, user.id, foundDelivery.delivery_agent._id);
      }
  ...
  async afterDeliveryStarted(delivery, user, preCheck = false) {
    try {
      let scannedItems = await super.getDeliveryAgentOrderLines(user.id);
      ...
      for (let i = 0; i < matchedItems.length; i++) {
        let order = {_id: mongoose.Types.ObjectId(matchedItems[i].order_id)};
        let orderLine = {_id: mongoose.Types.ObjectId(matchedItems[i].order_line_id)};

        await super.setOrderLineAsReadyToDeliver(order, orderLine, user.id, user.id);
      }
  ...
  async afterDeliveryEnded(delivery, user) {
    try {
      ...
      for (let i = 0; i < delivery.order_details.length; i++) {
        ...
        for (let j = 0; j < order.order_line_ids.length; j++) {
          let orderLine = {_id: order.order_line_ids[j]};
  ...
  async makeDelivery(order, orderLine, warehouses, user) {
    try {
      ...
      let delivery = await new DeliveryModel(this.test).initiate(
        order,
        orderLine._id,
      ...
      return new DeliveryModel(this.test).makeDeliveryShelfCode(delivery._id);
