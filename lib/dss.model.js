...
class DSS extends Ticket {
    ...
    try {
      ...
      if ([
        ...
        if (foundOrderLine.cancel) {
          ...
          new Offline(this.test).transferRequest(
            order._id.toString(),
            foundOrderLine._id.toString(),
            product.instance.barcode,
            user,
            _const.TRANSFER_TYPE.WarehouseToOnline)
        ...
        if (foundOrderLine.cancel) {
          new Offline(this.test).transferRequest(
            ...
            product.instance.barcode,
            user,
            _const.TRANSFER_TYPE.OnlineToWarehouse);
  ...
  async onCanceledNewOrderLineScan(order, orderLine, user) {
    try {
      ...
      let productInstance = await new ProductModel(this.test).getInstance(orderLine.product_id.toString(), orderLine.product_instance_id.toString())
    ...
    try {
      ...
      if (user.warehouse_id !== hub._id.toString()) // scan in central warehouse or other shops
      {
        ...
        if (isReturn) {
          ...
          await this.setOrderLineReturnType(order, orderLine, productInstance, user, _const.RECEIVE_TYPE.Return)
        }
        return this.makeDeliveryToCentral(order, orderLine, user, hub._id);
      }
  ...
  async setOrderLineReturnType(order, orderLine, productInstance, user, returnType) {
    await new Offline(this.test).requestForReturn(
      order,
      orderLine,
      productInstance.barcode,
      user,
      returnType);
    await super.setOrderLineAsWatingForReturn(order, orderLine, user.id, user.warehouse_id);
  }


  async makeDeliveryToCentral(order, orderLine, user, hubId) {
    let foundDelivery = await new DeliveryModel(this.test).makeReturnToCentral(order, orderLine, user);

    await super.setOrderLineAsDeliverySet(order, orderLine, user.id, hubId);
    if (foundDelivery.tickets[foundDelivery.tickets.length - 1].status === _const.DELIVERY_STATUS.requestPackage)
      await super.setOrderLineAsFinalCheck(order, orderLine, user.id, hubId);

  }

  async afterWarehouseToOnlineTransfer(order, orderLine, userId, warehouseId) {

    try {

      const lastTicket = this.getLastTicket(orderLine);

      if (lastTicket.status === _const.ORDER_LINE_STATUS.OnlineWarehouseVerified)
      ...
      orderLine = (await super.setOrderLineAsVerfiedByOnlineWarhouse(order, orderLine, userId, warehouseId)).orderLine;

      let index = order.order_lines.findIndex(x => x._id.toString() === orderLine._id.toString());
      order.order_lines[index] = orderLine;
      await this.afterSoldOut(order, orderLine, userId, warehouseId);
  ...
  async afterOnlineToWarehouseTransfer(order, orderLine, userId, warehouseId) {

    try {

      const lastTicket = this.getLastTicket(orderLine);

      if (lastTicket.status === _const.ORDER_LINE_STATUS.OnlineWarehouseCanceled)
        return Promise.resolve('online warehouse has canceled order line before')

      await new ProductModel(this.test).setInventory(orderLine.product_id, orderLine.product_instance_id, warehouseId, null, 1, null, null);

      await super.setOrderLineAsCanceled(order, orderLine, userId, warehouseId);
      if (!this.test)
  ...
  async afterReturnVerified(order, orderLine, userId, hubId) {
    try {

      let lastTicket = this.getLastTicket(orderLine);
      if (_const.ORDER_LINE_STATUS.ReturnVerified === lastTicket.status)
        return Promise.resolve('order line is previously returned by offline system');


      await super.setOrderLineAsReturnVerified(order, orderLine, userId, hubId);

      let CustomerModel = require('./customer.model');

      let hasDamageWithRefund = orderLine.tickets.map(x => x.status).includes(_const.ORDER_LINE_STATUS.DamageWithRefund);
      let hasDamageWithoutRefund = orderLine.tickets.map(x => x.status).includes(_const.ORDER_LINE_STATUS.DamageWithoutRefund);

      if (!hasDamageWithoutRefund) {
        let addedBalance = order.total_amount || 0 + order.used_balance || 0;
        await new CustomerModel(this.test).changeBalance(order.customer_id, addedBalance);
      }



      // only the normal returned order lines should be back to central warehouse
      if (!hasDamageWithRefund && hasDamageWithoutRefund) {
        await this.makeDeliveryToCentral(order, orderLine, userId, hubId)
      }

      if (!this.test)
        socket.sendToNS(hubId)

    } catch (err) {
      console.log('-> error on after return verification', err);
      throw err;
    }
  }
  ...
  async makeInvoice(order, orderLine, user) {
    try {
      ...
      if (isAggregated) {
        ...
        return new Offline(this.test).invoiceRequest(order, user)
      }
  ...
  async afterInvoiceVerification(orderId, invoiceId, userId, warehouseId, points) {
    ...
    try {
      ...
      let CustomerModel = require('./customer.model');
      await new CustomerModel(this.test).updateLoyalty(foundOrder.customer_id, points)
  ...
  async afterDamageInformed(order, orderLine, user, type) {
    try {

      if (!orderLine.tickets.find(x => x.status === _const.ORDER_LINE_STATUS.ReturnRequested))
        throw new Error('informing damage receive is only allowed for returned order lines');

      let smMessageType;
      if (type === _const.RECEIVE_TYPE.DamageWithRefund) {
        await super.setOrderLineAsDamageWithRefund(order, orderLine, user.id, user.warehouse_id);
        smMessageType = _const.SM_MESSAGE.DamageWithRefund;
      } else if (type === _const.RECEIVE_TYPE.DamageWithoutRefund) {
        await super.setOrderLineAsDamageWithoutRefund(order, orderLine, user.id, user.warehouse_id);
        smMessageType = _const.SM_MESSAGE.DamageWithoutRefund;
      } else
        throw new Error('type of damage is not defined');


      let SMMessage = require('./sm_message.model');
      await new SMMessage(this.test).pubishMessage(order._id, orderLine._id, smMessageType);
      ...
      await this.setOrderLineReturnType(order, orderLine, productInstance, user, type);
  ...
  async afterLostReport(order, orderLine, user) {
    try {
      ...
      if (!orderLine.tickets.find(x => x.status === _const.ORDER_LINE_STATUS.WaitForLost)) {
        let SMMessage = require('./sm_message.model');
      ...
      let transferType;
      if (orderLine.tickets.find(x => x.status === _const.ORDER_LINE_STATUS.WaitForOnlineWarehouse))
        transferType = _const.TRANSFER_TYPE.OnlineToLost;
      else
        transferType = _const.TRANSFER_TYPE.WarehouseToLost;

      await super.setOrderLineAsWaitingForLost(order, orderLine, user.id, user.warehouse_id);

      new Offline(this.test).transferRequest(
        order._id,
        orderLine._id,
        foundInstance.barcode,
        user,
        transferType);
      ...
      console.log('-> ', 'after lost report');
      throw err;
    }
  }

  async afterLostVerified(order, orderLine, userId, warehouseId) {
    try {

      let WarehouseModel = require('./warehouse.model');
      let warehouses = await new WarehouseModel(this.test).getWarehouses();

      await super.setOrderLineAsLostVerified(order, orderLine, userId, warehouseId);
      ...
      if (!isReturn) {
        ...
        await this.renewLostOrderLine(order, orderLine, warehouses);
      }

      let DeliveryModel = require('./delivery.model');
      await new DeliveryModel(this.test).safeRemoveOrderLineFromDelivery(orderLine._id);
  ...
  async afterUpdateInventoryRequest(barcode, warehouse, count) {

    const productModel = new ProductModel(this.test);
    const result = await productModel.getInstanceByBarcode(barcode);

    return productModel.setInventory(result._id, result.instance._id, warehouse._id, count);
  ...
  async syncLostInventory(orderLine, warehouses, warehouseId) {
    try {
      ...
      /**
       * warehouses does not include hub
       */
