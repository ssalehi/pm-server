            ...
            .then(res => {
              ...
              if (order.is_collect && dest._id.toString() === warehouseId.toString())
                ...
                return Promise.all(order.order_lines
                  .filter(el => el.tickets.length && el.tickets.slice(-1)[0].status === _const.ORDER_STATUS.WaitForOnlineWarehouse && el.tickets.slice(-1)[0].receiver_id.toString() === warehouseId.toString())
    ...
    const checkAllExceptInDist = () => {
      return Promise.resolve(order.order_lines.every(ol => {
        ...
        return lastTicket.receiver_id.toString() === hub._id.toString() ||
          lastTicket.receiver_id.toString() === destination._id.toString();  // all order lines are whether in current warehouse or in destination)
        }))
      }
