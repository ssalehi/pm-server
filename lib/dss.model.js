            ...
            .then(res => {
              ...
              if (order.is_collect && dest._id.toString() === warehouseId.toString())
                ...
                return Promise.all(order.order_lines
                  .filter(el => el.tickets.slice(-1)[0].status === _const.ORDER_STATUS.WaitForOnlineWarehouse && el.tickets.slice(-1)[0].receiver_id.toString() === warehouseId.toString())
          ...
          return this.checkOrderAggregation(order, orderLine, hub._id)
            ...
            .then(data => {
              ...
              return new Delivery(this.test).initiate(
                ...
                warehouseId ? {warehouse_id: warehouseId._id} : {customer: {_id: order.customer_id, address_id: order.address._id}});
            })
            .then(res => {
