...
class DSS extends Ticket {
  ...
  async afterFinalCheck(order, product, user, isExternal = false) {
    try {
      if (!order)
        throw error.orderNotFound;
      ...
      if (!order.is_collect) {
        ...
        if (!isExternal)
          ...
          if (isAggregated) {
            ...
            let Offline = require('./offline.model');
            await new Offline(this.test).requestInvoice()
          }
        }
      }
      if (!this.test)
  ...
  async afterInvoiceVerification(orderId, invoiceNo, userId, warehouseId, points) {
    ...
    try {
      ...
      let isAggregated = this.checkOrderAggregation(foundOrder, warehouseId);
    ...
    try {
      ...
      const checkAll = (targetId) => {
        ...
        return Promise.resolve(order.order_lines.every(ol => {
          ...
          return lastTicket.receiver_id.toString() === targetId.toString();
        }));
      }
