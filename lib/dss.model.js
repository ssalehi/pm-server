...
class DSS extends Ticket {
  ...
  async afterInboxScan(order, product, user) {
    try {
      ...
      } else if (lastTicket.status === _const.ORDER_LINE_STATUS.Cancel) {
  ...
  async afterOnlineWarehouseVerification(orderId, orderLineId, userId, warehouseId) {
    ...
    try {
      ...
      await new ProductModel(this.test).setInventory({
  ...
  async afterCancel(order, orderLine, user) {
    try {
      ...
      if ([
        ...
        await super.setOrderLineAsCanceled(order, orderLine, user.id, lastTicket.receiver_id) // set ticket for where order line is currently in  
        if (!this.test)
          socket.sendToNS(lastTicket.receiver_id);
      }
    } catch (err) {
      console.log('-> error on after cancel ', err);
      throw err;
    }
  }
