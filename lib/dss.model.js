...
class DSS extends Ticket {
    ...
    try {
      ...
      if (user.warehouse_id !== hub._id.toString()) // scan in central warehouse or other shops
      {
        ...
        if (isReturn) {
          ...
          new Offline(this.test).requestForReturn(
            order,
            orderLine,
            productInstance.barcode,
            user,
            _const.RECEIVE_TYPE.Return);

          return super.setOrderLineAsWatingForReturn(order, orderLine, user.id, user.warehouse_id);
        }
  ...
  async afterToLostTransfer(order, orderLine, warehouseId) {
    try {
  ...
  async afterReturnVerified(order, orderLine, userId, warehouseId) {
    try {
  ...
  async afterReturnDamagedVerified(order, orderLine, userId, warehouseId) {
    try {
      ...
      if ([
        ...
        return Promise.resolve('order line is previously verified as damaged');
      ...
      if (lastTicket.status === _const.ORDER_LINE_STATUS.WaitForDamageWithRefund) {
        await super.setOrderLineAsDamageWithRefundVerified(order, orderLine, userId, salesManager._id);
        let OrderModel = require('./order.model');
        let addedBalance = await new OrderModel(this.test).calculateDetailedPrice(order, orderLine);
        if (order.customer_id) {
          let CustomerModel = require('./customer.model');
      ...
      } else if (lastTicket.status === _const.ORDER_LINE_STATUS.WaitForDamageWithoutRefund)
        await super.setOrderLineAsDamageWithoutRefundVerified(order, orderLine, userId, salesManager._id);
  ...
  async afterRetuenLostVerified(order, orderLine, userId, warehouseId) {
    try {

      let warehouses = await new WarehouseModel(this.test).getWarehouses();

      if (warehouses.find(x => x._id.toString() === warehouseId.toString()))
        await new ProductModel(this.test).setInventory(orderLine.product_id, orderLine.product_instance_id, warehouseId, null, -1, null, null);
      ...
      console.log('-> error on after receive warehouse', err);
      throw err;
    }
  }
  ...
  async afterDamageInformed(order, orderLine, user, type) {
    try {
      ...
      if (type === _const.RECEIVE_TYPE.DamageWithRefund) {
        await super.setOrderLineAsWaitForDamageWithRefund(order, orderLine, user.id, user.warehouse_id);
        smMessageType = _const.SM_MESSAGE.DamageWithRefund;
      } else if (type === _const.RECEIVE_TYPE.DamageWithoutRefund) {
        await super.setOrderLineAsWaitForDamageWithoutRefund(order, orderLine, user.id, user.warehouse_id);
  ...
  async afterLostReport(order, orderLine, user) {
    try {
      ...
      if (!orderLine.tickets.find(x => x.status === _const.ORDER_LINE_STATUS.ReturnRequested)) { // lost report of not returned order line
        let transferType;
      ...
      } else { // lost report of returned order line
