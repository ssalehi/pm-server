...
class DSS extends Ticket {
  ...
  async resetOrderLineToLastTicket(order, orderLine) {
    try {
      ...
      if (lastDeliveredTicket) {
        receiever = lastDeliveredTicket.receiver_id;
        await super.setOrderLineAsDelivered(order, orderLine, order.customer_id, receiever);
  ...
  async afterToDamageLostTransfer(orderId, orderLineId, warehouseId) {
    try {
