...
class DSS extends Ticket {
  ...
  async onCanceledNewOrderLineScan(order, orderLine, user) {
    try {
      ...
      const lastTicket = getLastTicket(orderLine);
  ...
  async afterLost(order, orderLine, user) {
    try {
      ...
      await super.setOrderLineAsWatingForLostWarehouse(order, foundOrderLine, user.id, user.warehouse_id);
  ...
  async afterLostVerification(orderId, orderLineId, userId, warehouseId) {
    try {
      ...
      // inventory count is not reduced before, decrease count and reserved of inventory by 1
      ...
      await this.ORP(foundOrder, foundOrderLine, warehouses, null, true);
