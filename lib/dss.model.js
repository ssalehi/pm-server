          ...
          return Promise.all(foundOrder.order_lines.map(orderLine => new TicketModel(DSS.test).setTicket(foundOrder, orderLine, _const.ORDER_STATUS.ReadyToDeliver, userId, warehouseId)))
            .then(() => {
              return new Delivery(DSS.test).initiate(
                foundOrder,
                foundOrder.order_lines.map(ol => mongoose.Types.ObjectId(ol._id)),
                {warehouse_id: warehouseId},
                {
                  customer: {
                    _id: foundOrder.customer_id,
                    address_id: foundOrder.address._id,
                  }
                });
            })
            .then(data => {
              if (warehouseId.toString() === hub._id.toString())
                return new Delivery(DSS.test).makeDeliveryShelfCode(data._id);
            ...
            .then(res => {
              ...
              if (order.is_collect && dest._id.toString() === warehouseId.toString())
                ...
                return Promise.all(order.order_lines.map(ol => new TicketModel(DSS.test).setTicket(order, ol, _const.ORDER_STATUS.ReadyToDeliver, userId, warehouseId)))
                  .then(() => {
                    return new Delivery(DSS.test).initiate(order, [orderLine], {warehouse_id: warehouseId}, {warehouse_id: dest._id.toString()});
                  })
                  .then(data => {
                    if (res.find(el => el.is_hub)._id.toString() === warehouseId.toString())
                      return new Delivery(DSS.test).makeDeliveryShelfCode(data._id);
                    return Promise.resolve();
                  });
            });
                ...
                if (order.is_collect) {
                  return Promise.all(order.order_lines.map(ol => TicketModel.setTicket(order, ol, _const.ORDER_STATUS.ReadyToDeliver, user.warehouse_id, user.warehouse_id)))
                    .then(data => {
                      return new Delivery(DSS.test).initiate(
                        order,
                        order.order_lines.map(ol => mongoose.Types.ObjectId(ol._id))),
