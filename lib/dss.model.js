...
class DSS extends Ticket {
    ...
    try {
      ...
      if ([
        ...
        if (foundOrderLine.cancel) {
          ...
          await new Offline(this.test).transferRequest(
            order._id.toString(),
  ...
  async afterReceiveDamaged(order, orderLine, userId, warehouseId) {
    try {
  ...
  async afterReceiveLost(order, orderLine, userId, warehouseId) {
    try {
  ...
  async afterReceiveWarehouse(barcode, count, warehouse) {
    try {

      let productModel = new ProductModel(this.test);
      let product = await productModel.getInstanceByBarcode(barcode);

      await new ProductModel(this.test).setInventory(product._id, product.instance._id, warehouse._id, count, null, null, null);

      if (!this.test)
        socket.sendToNS(warehouse._id);
  ...
  async afterDamageInformed(order, orderLine, user, type) {
    try {
      ...
      await new Offline(this.test).receiveRequest(
        order._id,
        orderLine._id,
