...
class DSS extends Ticket {
  ...
  async afterInboxScan(order, product, user) {
    try {
      ...
      let central = (await warehouseModel.getAll()).find(x => !x.has_customer_pickup);
      ...
      let returnToCentral = async (o, ol) => {
        ...
        let foundDelivery = await new DeliveryModel(this.test).initiate(o, ol,
          {
        ...
        if (foundOrderLine.cancel) {
          if (lastTicket.status === _const.ORDER_LINE_STATUS.WaitForOnlineWarehouse) {
            ...
            let data = {
              ...
              warehouseId: lastTicket.receiver_id,
              delReserved: -1,
              delCount: 0
            }
            await new ProductModel(this.test).setInventory(data);
            await super.setOrderLineAsCanceled(order, foundOrderLine, user._id, lastTicket.receiver_id);
          }
        }
        else {
    ...
    try {
      ...
      if (isExternal && !isExternalRetrun) {
        ...
        for (let i = 0; i < foundOrders.length; i++) {
          ...
          let lastTicket = foundOrders[i].tickets[foundOrders[i].tickets.length - 1];
  ...
  async afterCancel(order, orderLine) {
    try {
      ...
      let foundDelivery = await deliveryModel.getActiveDeliveryByOrderLineId(orderLine._id);
      if (foundDelivery) {
        ...
        await deliveryModel.removeOrderLineFromDelivery(foundDelivery, orderLine._id);
      }
      ...
      if (lastDeliveredTicket) {
        ...
        receiever = verificationTicket.receiver_id;
