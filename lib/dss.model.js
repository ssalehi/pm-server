...
class DSS extends Ticket {
  ...
  async afterSoldOut(order, orderLine, userId, warehouseId) {
    try {
      ...
      if (!order.is_collect) {
        ...
        if (isAggregated) {
          for (let i = 0; i < order.order_lines.length; j++) {
  ...
  async afterFinalCheck(order, product, user, isExternal = false, isCC = false) {
    try {
      if (!order)
        throw error.orderNotFound;
      ...
      if (!order.is_collect) {
        ...
        if (!isExternal)
          ...
          order = (await super.setOrderLineAsChecked(order, foundOrderLine, user.id, user.warehouse_id)).order;
          ...
          if (isAggregated) {
            ...
            let Offline = require('./offline.model');
            await new Offline(this.test).requestInvoice(order, user)
          }
        }
      }
      if (!this.test)
