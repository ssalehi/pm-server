...
class DSS extends Ticket {
  ...
  async afterReceiveDamaged(order, orderLine, userId, warehouseId) {
    try {
      ...
      if (lastTicket.status === _const.ORDER_LINE_STATUS.WaitForDamageWithRefund) {
        ...
        if (order.customer_id) {
          let CustomerModel = require('./customer.model');
          await new CustomerModel(this.test).changeBalance(foundOrder.customer_id, addedBalance);
        }
  ...
  async afterReceiveWarehouse(order, orderLine, userId, warehouseId) {
    try {

      await new ProductModel(this.test).setInventory(orderLine.product_id, orderLine.product_instance_id, warehouseId, null, 1, null, null);
      await super.setOrderLineAsReturned(order, orderLine, userId, warehouseId);
  ...
  async afterLostReport(order, orderLine, user) {
    try {
      ...
      await super.setOrderLineAsWaitingForLostWarehouse(order, orderLine, user.id, user.warehouse_id);
