...
class DSS extends Ticket {
  ...
  async afterOnlineWarehouseVerification(orderId, orderLineId, userId, warehouseId) {
    ...
    try {
      ...
      const lastTicket = foundOrderLine.tickets && foundOrderLine.tickets.length ? foundOrderLine.tickets[foundOrderLine.tickets.length - 1] : null;
      if (!lastTicket || lastTicket.is_processed ||
        (lastTicket.status !== _const.ORDER_LINE_STATUS.WaitForOnlineWarehouse && lastTicket.status !== _const.ORDER_LINE_STATUS.OnlineWarehouseVerified))
  ...
  async afterRequestForPackage(delivery, userId, isExternal, isRetrun) {
    try {
      ...
      if (isExternal && !isRetrun) {
        let allAggregated = foundOrders.every(x => this.checkOrderAggregation(x, delivery.from.warehouse_id));
        if (!allAggregated)
  ...
  async afterFinalCheck(order, product, user, isExternal = false, isDestination = false, isReturn = false) {
    try {
      if (!order)
        throw error.orderNotFound;
      ...
      if (!isReturn && !isExternal) {

        if (isDestination) { // cc delivery
          if (!order.is_collect)
            throw new Error('only cc orders are allowed in final scan of destination');
          ...
          if (user.warehouse_id !== hub._id.toString())
            ...
            if (!order.is_collect)
              throw new Error('order cannot be delivered internally to another warehouse in case of not return or not cc')
            to = {
              warehouse_id: order.address.warehouse_id
            };
          }
      ...
      else if (!isReturn && order.is_collect && isDestination) {
        await this.makeInvoice(order, foundOrderLine, user);
      } else if (!isReturn && order.is_collect && !isDestination) {
        let hub = await new WarehouseModel(this.test).getHub();
        let to = {
          warehouse_id: user.warehouse_id === hub._id.toString() ? order.address.warehouse_id : hub._id
        };
  ...
  async afterDeliveryStarted(delivery, user, preCheck = false, isExternal = false, isRetrun = false) {
    try {
      ...
      let scannedItems = await super.getDeliveryAgentOrderLines(receiverId, isExternal);
      ...
      if (!isExternal) {
        ...
        //   await super.revert(foundOrder, foundOrderLine, false, 2); // revert to 2nd last ticket

        // }
      } else {
        if (excludeItems && excludeItems.length)
          throw new Error('some order lines has not final checked yet');
        ...
        for (let i = 0; i < delivery.order_details.length; i++) {
          ...
          if (!isRetrun) {
            let isAggregated = await this.checkOrderAggregation(foundOrder, delivery.from.warehouse_id);
            if (!isAggregated)
              throw new Error('delivery contains not aggregated orders');
          }
  ...
  async afterDeliveryEnded(delivery, user, isExternal, isReturn) {
    try {
  ...
  async afterReceive(order, orderLine, user, isReturn = false) {
    try {
      ...
      if (isReturn) {
        ...
        if (![hub, central].map(x => x._id.toString()).includes(user.warehouse_id))
      ...
      if (hub._id.toString() === user.warehouse_id) { // in hub
        ...
        let isAggregated = await this.checkOrderAggregation(order, orderLine, [
          _const.ORDER_LINE_STATUS.OnlineWarehouseVerified,
          _const.ORDER_LINE_STATUS.Recieved
        ]);
      ...
      } else if (user.warehouse_id === central._id.toString()) {

        if (!isReturn)
          throw new Error('central warehouse can receive order line only at return case');
