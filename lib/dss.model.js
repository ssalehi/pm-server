...
class DSS extends Ticket {
  ...
  async afterWarehouseToOnlineTransfer(orderId, orderLineId, userId, warehouseId) {
  ...
  async afterOnlineToWarehouseTransfer(orderId, orderLineId, userId, warehouseId) {
  ...
  async afterToDamageLostTransfer(orderId, orderLineId, warehouseId, type) {
    try {
  ...
  async afterDamageInformed(order, orderLine, user) {
    try {
      ...
      await super.setOrderLineAsWaitForDamage(order, orderLine, user.id, salesManager._id);
  ...
  async afterLostReport(order, orderLine, user) {
    try {
      ...
      if (orderLine.tickets.find(x => x.status === _const.ORDER_LINE_STATUS.WaitForOnlineWarehouse))
        transferType = _const.TRANSFER_TYPE.OnlineToDamageLost;
      else
        transferType = _const.TRANSFER_TYPE.WarehouseToDamageLost;
