...
class DSS extends Ticket {
  ...
  async afterSoldOut(order, orderLine, userId, warehouseId) {
    try {
      ...
      if (!order.is_collect || (order.is_collect && order.address.warehouse_id.toString() !== warehouseId.toString())) {
        ...
        if (delivery.tickets[delivery.tickets.length - 1] === _const.DELIVERY_STATUS.requestPackage) {
  ...
  async afterReceive(order, orderLine, user) {
    try {
      ...
      if (!order.is_collect) {
        ...
        if (user.warehouse_id.toString() === hub._id.toString())
  ...
  async checkOrderAggregation(order, warehouseId, status = null) {
    ...
    try {
      ...
      const checkAll = (targetId) => {
        ...
        return Promise.resolve(order.order_lines.every(ol => {
          ...
          if (status)
            conidtion = conidtion && lastTicket.status === status;
