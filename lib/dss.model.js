...
class DSS extends Ticket {
    ...
    try {
      ...
      if ([
        ...
        if (user.warehouse_id === hub._id.toString()) { // order line must be returned to central warehouse
          ...
          let foundDelivery = await deliveryModel.initiate(order, foundOrderLine,
            {
          ...
          if (foundDelivery.tickets[foundDelivery.tickets.length - 1].status === _const.DELIVERY_STATUS.requestPackage)
            await super.setOrderLineAsFinalCheck(order, foundOrderLine, user.id, hub._id);
        }
    ...
    try {
      ...
      if (!isExternal) {
        ...
        for (let i = 0; i < delivery.order_details.length; i++) {
          ...
          if (isExternal && !isExternalRetrun &&
            foundOrder.tickets[foundOrder.tickets.length - 1].status !== _const.ORDER_STATUS.ReadyToDeliver) // if delivery is to customer, orders must be ready to deliver (after invoice)
          ...
          let isAggregated = await this.checkOrderAggregation(foundOrder, delivery.from.warehouse_id,
            (isExternal && !isExternalRetrun) ? [_const.ORDER_LINE_STATUS.Checked] : null); // if delivery is to customer, all order lines must be checked
          if (!isAggregated)
            throw new Error('delivery contains not aggregated orders');
  ...
  async afterExternalReturnReceive(order, orderLine, user) {
    try {
