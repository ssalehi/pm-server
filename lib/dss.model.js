...
class DSS {
  ...
  ORP(order, order_line, warehouses, exceptionId = null) {
    if (exceptionId && warehouses.filter(el => el.has_customer_pickup).includes(exceptionId))
      ...
      .then(productInstance => {
        ...
        if (!warehouse) {
          ...
          return new Ticket(this.test).setAsReserved(order, order_line, warehouse)
            .then(res => {
              if (!order.is_collect)
                return new Delivery(DSS.test).initiate(order, order_line._id, {warehouse_id: warehouse._id.toString()}, {warehouse_id: warehouses.find(el => el.is_hub)._id.toString()});
  ...
  afterInvoiceVerification(orderId, invoiceNo, userId, warehouseId) {
    ...
    return new WarehouseModel(this.test).getAll()
      .then(res => {
        ...
        if (warehouse_id.toString() === hub._id.toString() && order.is_collect)
          return Promise.reject(error.noAccess);
      ...
      .then(res => {
        return Promise.all(foundOrder.order_line.map(orderLine => new TicketModel(DSS.test).setTicket(foundOrder, orderLine, _const.ORDER_STATUS.InvoiceVerified, userId, warehouseId)))
      })
      .then(res => {
        if (foundOrder.is_collect) {
          return Promise.all(foundOrder.order_line.map(orderLine => new TicketModel(DSS.test).setTicket(foundOrder, orderLine, _const.ORDER_STATUS.Delivered, userId, warehouseId, null, true)))
        } else {
      ...
      .then(res => {
        ...
        if (user.warehouse_id === hub._id.toString()) { // is hub
          ...
          return this.checkOrderAggregation(order, orderLine, hub._id)
            ...
            .then(data => {
              return new Delivery(DSS.test).initiate(
                order,
                [orderLine._id],
                {warehouse_id: user.warehouse_id},
                {warehouse_id: res.find(el => el.address._id.toString() === order.address._id.toString())._id});
            })
            .then(res => {
              return new Delivery(DSS.test).makeDeliveryShelfCode(res._id);
            })
  ...
  checkOrderAggregation(order, orderLine, warehouseId) {
    ...
    const checkAllInWarehouse = () => {
      ...
      return Promise.resolve(order.order_lines.filter(ol => {
        ...
        .every(ol => lastTicket.status === _const.ORDER_STATUS.WaitForAggregation))
    }
