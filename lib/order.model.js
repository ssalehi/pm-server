...
class Order extends Base {
  ...
  prepareDataForBankGateway(user, data) {
    ...
    data.total_amount = 1000; // only for test, should be remove later
    ...
    // return Promise.resolve(bankDataObj);
      ...
      rp({
        ...
        .then(async res => {
          parseString(res.body, function (err, result) {
            xmlToNodeReadRes = result;
          });

          if (xmlToNodeReadRes.resultObj.result[0] === 'False') {
            if (xmlToNodeReadRes.resultObj.action[0] === 'NOTRANSACTON') {
              console.log('NO TRANSACTION!!');
            }
            console.log('Shop Unsuccessful');
            return Promise.reject(xmlToNodeReadRes);
          }

          if (xmlToNodeReadRes.resultObj.result[0] === 'True') {
            if (xmlToNodeReadRes.resultObj.terminalCode[0] === env.terminal_code.toString() &&
              xmlToNodeReadRes.resultObj.merchantCode[0] === env.merchant_code.toString() &&
              xmlToNodeReadRes.resultObj.action[0] === actionObj.shop &&
              xmlToNodeReadRes.resultObj.invoiceDate[0] === data.invoiceDate.toString() &&
              xmlToNodeReadRes.resultObj.invoiceNumber[0] === data.invoiceNumber.toString()) {

              //should final check for last time
              let orderObj = await this.OrderModel.findOne({
                invoice_number: data.invoiceNumber
                , is_cart: true
              });

              orderObj = orderObj.order_lines;


              orderObj.forEach(el => cartItems.push({
              ...
              orderObj.forEach(p => {
                ...
                if (productInsIds.findIndex(p.product_instance_id) === -1)
                  productInsIds.push(p.product_instance_id);
              });
              console.log(productInsIds);
              productInsIds.forEach(el => {
                let q = cartItems.filter(c => c.product_instance_id === el).length;
                cartItems.filter(item => item.product_instance_id === el)[0].quantity = q;
              });

              return (new ProductModel()).getProducts(productIds, undefined, undefined, undefined, true);

              // resolve(xmlToNodeReadRes);
            } else {
        ...
        .then(res => {
          ...
          return Promise.resolve(xmlToNodeReadRes);
        })
        .catch(err => {
          reject(err)
        });
  ...
  verifyPayment(user, data) {
    ...
    let dataArr = ["",
      ...
      data.invoiceNumber,
      data.invoiceDate,
      data.xmlToNodeReadRes.amount[0],
      time,
      ""];
    const signedString = this.generateDigitalSignature(dataArr);
    let shopConfirmData = {
      ...
      invoiceNumber: data.invoiceNumber,
      invoiceDate: data.invoiceDate,
      amount: data.xmlToNodeReadRes.amount[0],
      timeStamp: time,
      sign: signedString,
    };
    ...
    return rp({
      ...
      parseString(res.body, function (err, result) {
        xmlToNodeVerifyRes = result;
      });

      if (xmlToNodeVerifyRes.actionResult.result[0] == 'True') {
        ...
        return this.OrderModel.findOneAndUpdate({
            ...
            $set: {
              is_cart: false, transaction_id: data.tref, IPG_data: {
                result: data.xmlToNodeReadRes.result[0],
                action: data.xmlToNodeReadRes.action[0],
                invoice_number: data.invoiceNumber,
                invoice_date: data.invoiceDate,
                transaction_id: data.tref,
                trace_number: data.xmlToNodeReadRes.traceNumber[0],
                reference_number: data.xmlToNodeReadRes.referenceNumber[0],
                transaction_date: data.xmlToNodeReadRes.transactionDate[0],
