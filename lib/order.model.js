...
class Order extends Base {
  ...
  async shopFullyWithBalance(user, body, update) {
    try {
      if (!user)
        throw new Error('payment with balance is only available for logged in users');

      if (body.hasOwnProperty('paymentType') && body.paymentType !== _const.PAYMENT_TYPE.balance)
        throw new Error('cash type payment orders can not be payed using balance');
      ...
      if (update.used_balance && update.used_balance > 0) {
        let CustomerModel = require('./customer.model');
      ...
      return Promise.resolve({
        invoiceNumber,
        invoiceDate,
        usedBalance: update.used_balance,
      });
    } catch (err) {
      console.log('Err : ', err);
      throw err;
  ...
  async checkout(user, body) {
    try {
      ...
      // loyalty discount which should be received from offline system
      update.loyalty_discount = 0;
      ...
      if (body.paymentType === _const.PAYMENT_TYPE.balance) {
        ...
        if (customer.balance > 0) {
          if (customer.balance >= update.total_amount)
            return await this.shopFullyWithBalance(user, body, update);
          else {
