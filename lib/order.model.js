...
class Order extends Base {
  ...
  verifyOrder(orderId, addressId, transactionId) {
  ...
  setTicket(body) {
    ...
    if (!Object.values(_const.ORDER_STATUS).some(x => x === body.status))
    ...
    return this.OrderModel.findOne({
      ...
      'transaction_id': {$ne: null},
      'is_cart': false,
      'order_lines._id': mongoose.Types.ObjectId(body.order_line_id)
    })
    .then(res => {
      let results = [];
      res.order_lines.filter(el => el._id == body.order_line_id).map(el => {
        el.tickets.forEach(_el => {
          if(_el.is_processed) {
            results.push({ticket_id: _el._id, order_line_id: el._id})
          }
        });
      });
      ...
      if (!results.length) {
        return this.OrderModel.update({
          ...
          },
          {
            $addToSet: {
              'order_lines.$.tickets': {
                warehouse_id: mongoose.Types.ObjectId(body.warehouse_id),
                status: body.status,
                desc: body.desc,
                is_processed: false
              },
        ...
        return Promise.reject(error.ActiveTicketExist);
      }
    })
  }
