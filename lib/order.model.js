...
class Order extends Base {
  ...
  sendDataToBank(user, data) {
    ...
    let redirectAddress = "http://localhost:4200/checkout";
    let timeStamp = time;
    let invoiceNumber = mongoose.Types.ObjectId().toString();
    ...
    let dataString = "#" + env.merchant_code + "#" + env.terminal_code  + "#" + invoiceNumber
      + "#" + invoiceDate + "#" + data.total_amount + "#" + redirectAddress + "#" + action + "#"
      + timeStamp + "#";
    let sh1String = sha1(dataString);


    let pemKey = new RSAXML().exportPemKey(privateKey);
    const signKey = new NodeRSA(pemKey);
    console.log(signKey);
    const signedString = signKey.sign(sh1String, 'UTF-8', 'UTF-8');

    let buff = new Buffer.alloc(signedString).toString('base64');
    let finalSign = buff;
