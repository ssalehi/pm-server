...
class Order extends Base {
  ...
  sendDataToBank(user, data) {
    let merchantCode = env.merchant_code;
    let terminalCode = env.terminal_code;
    let invoiceNumber = mongoose.Types.ObjectId();
    let invoiceDate = '2018/12/09 10:33:00';
    ...
    if(user && user.access_level === undefined) {
      // return models()['Customer' + (Order.test ? 'Test' : '')].find({
    ...
    let dataToSign = "#" + merchantCode + "#" + terminalCode + "#" + invoiceNumber
    ...
    const signedString = signKey.sign(hashDataToSign).toString('base64');
    ...
    return rp({
      ...
      uri: 'https://pep.shaparak.ir/gateway.aspx',
      json: true,
      resolveWithFullResponse: true,
    }).then( res => {
      console.log('res : ', res.request.body);
