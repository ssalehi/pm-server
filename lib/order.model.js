      ...
      rp({
        ...
        .then(async res => {
          ...
          if (xmlToNodeReadRes.result[0] === 'False') {  // transaction is unsuccessful (result: false), no need to continue,
            ...
            if (xmlToNodeReadRes.action[0] === 'NOTRANSACTON') {
              console.log('NO TRANSACTION!!');
            }
            if(xmlToNodeReadRes.action[0] === 'NOINVOICE') {
              console.log('INVOICE NOT EXISTS!!');
            }
          ...
          if (xmlToNodeReadRes.result[0] === 'True') {  // transaction is unsuccessful (result: true), need to final checkout
            ...
            if (xmlToNodeReadRes.terminalCode[0] === env.terminal_code.toString() &&
              ...
              xmlToNodeReadRes.action[0] === actionObj.shop &&
              xmlToNodeReadRes.invoiceDate[0] === data.invoiceDate.toString() &&
              xmlToNodeReadRes.invoiceNumber[0] === data.invoiceNumber.toString()) {
        ...
        .then(res => {
          ...
          orderObj.forEach(p => {
            ...
            productInsIds.push(p.instance_id);
          });
