...
class Order extends Base {
  ...
  async checkout(user, body, demo) {
    try {
      ...
      //todo: if user pays through balance or loyality...
      ...
      let productsPricingResult = await this.calculateOrderPrice(update.order_lines, user, coupon_code);

      update.order_lines = productsPricingResult.orderLines;
      update.total_amount += productsPricingResult.totalProductPrice;
      ...
      if (body.paymentType !== _const.PAYMENT_TYPE.cash) {
        if (body.paymentType === _const.PAYMENT_TYPE.balance) {
          let CustomerModel = require('./customer.model');

          let customer = await new CustomerModel(this.test).getById(user.id);
          if (customer.balance > 0) {
            update.total_amount -= customer.balance
            update.used_balance = customer.balance;
          }
      ...
      if (demo) {
        if (!env.isDev)
          throw new Error('payment demo is only active in dev mode');

        update.transaction_id = `xy-${Math.floor(Math.random() * Math.floor(100000))}`;
  ...
  async calculateOrderPrice(orderLines, user, coupon_code) {


    let products = await (new ProductModel()).findOrderProductsInfo(orderLines.map(x => {
      return {
    ...
    orderLines.forEach(ol => {
      ...
      if (foundProdcut.campaignInfo && foundProdcut.campaignInfo.length) {
        ...
        let discount = Math.round(ol.product_price * ol.campaign_info.discount_ref / (100 * env.rounding_factor)) * env.rounding_factor;
        ol.paid_price = ol.product_price - discount;
      } else {
        ol.paid_price = ol.product_price;
      }

      totalProductPrice += ol.paid_price
    });

    if (coupon_code) {
      let foundCouponCampaign = await this.checkCouponValidation(user, {coupon_code})
  ...
  async readPayResult(user, data) {
    try {
      ...
      let notPayedOrder = await this.OrderModel.aggregate([
        {
          $match: {
            $and: [
      ...
      if (!notPayedOrder || notPayedOrder.length === 0)
      ...
      let xmlToNodeReadRes = await parseString(res.body);
      ...
      let xmlToNodeVerifyRes = await this.verifyPayment(user, data, xmlToNodeReadRes);
      return Promise.resolve({xmlToNodeReadRes, xmlToNodeVerifyRes});
  ...
  async verifyPayment(user, data, xmlToNodeReadRes) {
    try {
      ...
      let xmlToNodeVerifyRes = await parseString(res.body);
      if (xmlToNodeVerifyRes.result[0] !== 'True')
        throw new Error(xmlToNodeVerifyRes);

      let tempRes = await this.OrderModel.findOneAndUpdate({
      ...
      await dssModel.startProcess(tempRes);
  ...
  async calculateReturnPrice(order, orderLine, isCancel = false) {
    try {
