...
class Order extends Base {
  ...
  checkoutCart(user, cartItems, orderId, address, customerData, transaction_id, used_point, used_balance, total_amount, is_collect, discount, duration_days, time_slot, paymentType, loyalty) {
    const is_cart = false;
  ...
  earnSpentPoint(user, paymentType, total_amount, is_collect, recieved_point) {
    let earn_point = 0;
      ...
      .then(res => {
        ...
        if (paymentType === 2)  // user dos not earn any point when uses his/her loyalty points eather in C&C or not
          return Promise.resolve({
            'earn_point': user_point,
            'final_point': user_point,
          });
