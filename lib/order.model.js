    ...
    return new Promise((resolve, reject) => { // read transaction result
      rp({
        ...
        .then(async res => {
          ...
          if (xmlToNodeReadRes.result[0] === 'False') {
          ...
          if (xmlToNodeReadRes.result[0] === 'True') {
        ...
        .then(res => {
          ...
          if (res[0].total_amount.toString() !== xmlToNodeReadRes.amount[0])
            return Promise.reject(error.invalidInvoice);

          res[0].forEach(el => cartItems.push({
        ...
        .then(res => {
          ...
          if (soldOuts && soldOuts.length)
            return Promise.reject(error.soldOutBeforPayment);
