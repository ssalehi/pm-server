    ...
    return new Promise((resolve, reject) => { // read transaction result
      rp({
        ...
        .then(res => {
          ...
          if (xmlToNodeReadRes.resultObj.result[0] === 'False' && xmlToNodeReadRes.resultObj.action[0] === 'NOTRANSACTON') {
          ...
          if (xmlToNodeReadRes.resultObj.result[0] == 'True') {
            if (xmlToNodeReadRes.resultObj.terminalCode[0] === env.terminal_code.toString() &&
              ...
              console.log('call final check for last time');

              // should call final check for last time
              console.log(data.cartItems);
              return this.finalCheck(data.cartItems);
            }
            else {
        ...
        .then(res => {
          console.log('LAST FINAL CHECK RESULT', res);
