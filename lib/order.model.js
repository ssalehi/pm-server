...
class Order extends Base {
  ...
  //   this.customerModel = test ? models['CustomerTest'] : models['Customer'];
  // }
  ...
  verifyOrder(user, orderId, addressId, transactionId, usedPoints, usedBalance) {
    let sentAddress;
    let warehouse = new WarehouseModel(Order.test);


    if (!user)
      return Promise.reject(error.noUser);
    else {
      return new CustomerModel(Order.test).getById(user._id)
        .then(res => {
          this.sentAddress = res.addresses.find(el => el._id.toString() === addressId.toString());
          console.log('**********', this.sentAddress);
          return warehouse.getAllWarehouses()
        })
        .then(res => {
          let central = res.find(x => x.is_center);
          if (central) {
            return this.OrderModel.update({
              _id: orderId
            }, {
            ...
            return Promise.reject(error.centralWarehouseNotFound);
      ...
      [
        ...
        },
        {
          $project: {
            ...
            product_instance_id: '$order_lines.product_instance_id',
            // product: '$product',
    ...
    return this.checkCurrentStatus(type, body)
      .then(res => {
        ...
        if (res.isActive) {
          ...
          else // those tickets with implementation of refresh scenario. e.g: invoice
            refresh = true;
        }
        if (typeof this[type] === 'function') {

          // some tickets such as invoice needs to be refreshed (to send new request to offline system)
          return this[type](body, res.order, user, refresh);
        }
      });
  }
      ...
      .then(res => {
        if (res && res.n === 1 && res.nModified === 1) {
          ...
          let newTicketUpdate = {
            '$addToSet': {
              'order_lines.$.tickets': {
                'warehouse_id': order.is_collect ? order.address_id : mongoose.Types.ObjectId(body.warehouseId),
                'status': _const.ORDER_STATUS.SMAssignToWarehouse,
  ...
  invoice(body, order, user, refresh) {
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.output) {
      ticketQueryOne = {
        cmp_value: {$eq: 0},
        $and: [{'tickets.warehouse_id': mongoose.Types.ObjectId(user.warehouse_id)}, {'tickets.is_processed': true}]
      };
      ticketQueryTwo = {
      ...
      ticketQueryTwo = {
        $and: [{'tickets.warehouse_id': mongoose.Types.ObjectId(user.warehouse_id)}, {'tickets.is_processed': false}]
      };
    }
...
//       {
//         $group: {
//           _id: '$transaction_id', order_lines: {$push: '$$ROOT'}
//         }
//       },
//     ]).then(res => {
//     return Promise.resolve({
