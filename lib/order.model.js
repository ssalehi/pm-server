...
class Order extends Base {
  ...
  getOrders(user) {
    if (!user)
      return Promise.reject(error.noUser);
    console.log(user.id);
    // this.model.find({customer_id: mongoose.Types.ObjectId(user.id)},
    //     'orders name')
    //     .then(result => {
    //       console.log(result);
    //       this.model.find({_id: result}).then(ress => {
    //         console.log("169", ress);
    //       });
    //
    //
    ...
    return this.OrderModel.aggregate(
        [
          ...
          // {
          //   $unwind: {
          //     path: '$product.colors', // it makes product.instances, single element array for each instance
          //     preserveNullAndEmptyArrays: true
          //   }
          // },
          {
            $project: {
              // transaction_id: '$order_lines.transaction_id',
              total_amount: 1,
              used_point: 1,
              is_collect: 1,
