...
class Order extends Base {
  ...
  checkoutCart(user, cartItems, orderId, address, customerData, transaction_id, used_point, used_balance, total_amount, is_collect, discount, duration_days, time_slot, paymentType, recievedLoyalty) {
    ...
    if (user && user.access_level === undefined) {
      ...
      this.earnSpentPoint(user, paymentType, total_amount, is_collect, duration_days, recievedLoyalty, discount)
        .then(res => {
  ...
  earnSpentPoint(user, paymentType, total_amount, is_collect, duration_days, recievedLoyalty, discount) {
    ...
    return new CustomerModel().getById(user.id)
      .then(res => {
        ...
        earn_point = Math.floor((total_amount - discount) / system_offline_offer);
        ...
        if (paymentType === 2) { // user dos not earn any point when uses his/her loyalty points eather in C&C or not
          ...
          if (is_collect) {
            ...
            if (user_point_price > (total_amount - discount)) {
              return Promise.resolve({
                'earn_point': 0,
                'final_point': user_point - Math.floor((total_amount - discount) / shop_value_offer),
                'shop_spent': Math.floor((total_amount - discount) / shop_value_offer),
                'shop_value': total_amount - discount,
              })
            }
            else {
              return Promise.resolve({
