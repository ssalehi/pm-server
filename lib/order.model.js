...
class Order extends Base {
  ...
  readAndVerifyPayResult(user, data) {
    // data.tref = null; // should remove later
    let time = moment().format("YYYY/MM/DD HH:mm:ss").toString();
    let xmlToNodeReadRes;
    let xmlToNodeVerifyRes;
    let formData = null;
    // please DO NOT remove this comment-out lines until line 197, it will be used later
    // if (data && data.tref) {
    //   formData = {
    //     trefID: data.tref,
    //   }
    ...
    return new Promise((resolve, reject) => {
      rp({
        method: 'post',
        uri: 'https://pep.shaparak.ir/CheckTransactionResult.aspx',
        form: formData,
        headers: {
        ...
        .then(res => {
          ...
          if (xmlToNodeReadRes.resultObj.result[0] == 'True') {
            if (xmlToNodeReadRes.resultObj.terminalCode[0] === env.terminal_code.toString() &&
              ...
              xmlToNodeReadRes.resultObj.invoiceNumber[0] === data.invoiceNumber.toString()) { // start shop confirmation
              ...
              let dataArr = ["",
                ...
                data.invoiceNumber,
                data.invoiceDate,
                xmlToNodeReadRes.resultObj.amount[0],
                time,
                ""];
              const signedString = this.generateDigitalSignature(dataArr);
              let shopConfirmData = {
                ...
                invoiceNumber: data.invoiceNumber,
                invoiceDate: data.invoiceDate,
                amount: xmlToNodeReadRes.resultObj.amount[0],
                timeStamp: time,
                sign: signedString,
              };
              ...
              return rp({
                method: 'post',
                uri: 'https://pep.shaparak.ir/VerifyPayment.aspx',
          ...
          } else if (xmlToNodeReadRes.resultObj.result[0] == 'False') {
            console.log('Shop Unsuccessful');
            return Promise.reject(xmlToNodeReadRes);
          }
        })
        .then(res => {
          console.log('SHOP CONFIRM : ', res.body);
          parseString(res.body, function (err, result) {
          ...
          if (xmlToNodeVerifyRes.actionResult.result[0] == 'True') {
            ...
            return this.OrderModel.findOneAndUpdate({
                ...
                $set: {is_cart: false, transaction_id: data.tref, IPG_data: {
                    result: xmlToNodeReadRes.resultObj.result[0],
                    action: xmlToNodeReadRes.resultObj.action[0],
                    invoice_number: data.invoiceNumber,
                    invoice_date: data.invoiceDate,
                    transaction_id: data.tref,
                    trace_number: xmlToNodeReadRes.resultObj.traceNumber[0],
                    reference_number: xmlToNodeReadRes.resultObj.referenceNumber[0],
                    transaction_date: xmlToNodeReadRes.resultObj.transactionDate[0],
