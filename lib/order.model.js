...
class Order extends Base {
  ...
  search(options, offset, limit) {
    ...
    return this.OrderModel.aggregate(
      [
        {
          $match: {$and: [{is_cart: false}, {transaction_id: {$ne: null}}, {address_id: {$ne: null}}]}
        },
        {
          $lookup: {
            from: 'product',
            localField: 'order_line_ids.product_id',
            foreignField: '_id',
            as: 'product'
          }
        },
      ...
      result.forEach((item, index) => {
        if (item.is_collect) {
          models['Warehouse' + (Order.test ? 'Test' : '')].findById(mongoose.Types.ObjectId(item.address_id)).lean().then(warehouseRes => {
            result[index]['warehouseAddress'] = warehouseRes;
          });
        }
      });
      return this.OrderModel.aggregate(
