...
class Order extends Base {
  ...
  async addToOrder(user, items, updateQuery = {}, guest = false) {
    ...
    for (let item of items) {
      ...
      paid_price = await (new ProductModel()).findProductPrice(pid, piid);
      if(!paid_price || !paid_price.length)
        return Promise.reject(error.invalidId);
      ...
      for (let i = 0; i < n; i++) {
        newOrderLines.push({
          ...
          paid_price: paid_price[0].instances.price,
        });
      }
    }
  ...
  async calculateOrderPrice(user, orderId, cartItems) {
    ...
    if(user && user.access_level === undefined) {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.OrderModel.findOne({_id : mongoose.Types.ObjectId(orderId)})
          .then(res => {
            res.order_lines.forEach(el => total_amount += el.paid_price);
      ...
      for (let item of cartItems) {
        ...
        total_amount += (paid_price[0].instances.price * item.number);
      }
      return total_amount;
    }
  }
