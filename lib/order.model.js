...
class Order extends Base {
  ...
  checkoutCart(user, cartItems, orderId, address, customerData, transaction_id, used_point, used_balance, total_amount, is_collect, discount, duration_days, time_slot, selected_payment_type) {
    const is_cart = false;
    ...
    if (!is_collect && (!duration_days || !time_slot || !selected_payment_type)) {
  ...
  earnSpentPoint(user, paymentType, total_amount, is_collect) {
    let earn_point = 0;
    ...
    return new CustomerModel().getById(user._id)
      .then(res => {
        if (!res.length || res.is_guest)
          return Promise.resolve({'earn_point' : 0});
        ...
        if (paymentType === 2)  // user dos not earn any point when uses his/her loyalty points eather in C&C or not
          return Promise.resolve({'earn_point' :user_point});

        if (!is_collect) {
          return Promise.resolve({'earn_point' :earn_point});
        }
      ...
      .then(res => {
        ...
        return Promise.resolve({'earn_point' : earn_point + addedPoint});
      })
  };
