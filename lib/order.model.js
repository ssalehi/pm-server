...
class Order extends Base {
  ...
  ORP(order, order_line, warehouses, exceptionId = null, returnFunction = true) {
    ...
    return new ProductModel(Order.test).getInstance(order_line.product_id.toString(), order_line.product_instance_id.toString())
      .then(productInstance => {
        ...
        return this.setAutomatedTicket(order, order_line, warehouse, status);
      });
  }
  ...
  internalDelivery(body, order, user) {
    ...
    return this.closeCurrentTicket(body.orderId, body.orderLineId, user.id)
      .then(res => {
        ...
        return this.addNewTicket(body.orderId, body.orderLineId, _const.ORDER_STATUS.InternalDelivery, user.warehouse_id);
      })
      .then(res => {
        return this.pushMessage(user.warehouse_id, {
  ...
  receive(body, order, user) {
    ...
    return new WarehouseModel(Order.test).getAllWarehouses()
      .then(res => {
        let foundWarehouse = res.find(x => x._id.toString() === user.warehouse_id);
        if (!foundWarehouse)
          return Promise.reject(error.WarehouseNotFound);

        let centralWarehouse = res.find(x => x.is_center);

        requestInvoice = centralWarehouse._id.toString() !== foundWarehouse._id.toString();
