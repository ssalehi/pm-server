...
class Order extends Base {
  ...
  offlineInvoice(order, orderLineId, warehouseId, userId) {
    ...
    let foundOrderLine = order.order_lines.find(x => x._id === orderLineId);
    ...
    return new ProductModel(Order.test).getInstance(productId, productInstanceId)
      .then(res =>{
        productInstance = res;
        // return new
      })

    helpers.httpPost(env.offlineURL, {
      orderId,
      orderLineId,
      warehouseId,
      userId
