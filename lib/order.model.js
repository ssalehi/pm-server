...
class Order extends Base {
  ...
  async calculateOrderPrice(user, orderId, cartItems) {
    ...
    for (let item of order_line_array) {
      let pid = item.product_id;
      let piid = item.product_instance_id;
      let order_product = 0;

      order_product = await (new ProductModel()).findProductPrice(pid, piid);
      total_amount += (order_product[0].instances.price * (item.number ? item.number : 1));
      console.log(JSON.stringify(order_product));
    }
    return total_amount;
  }
  ...
  async prepareDataForBankGateway(user, data) {
    ...
    let total_amount = await this.calculateOrderPrice(user, data.order_id, data.cartItems);
