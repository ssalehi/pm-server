...
class Order extends Base {
  ...
  checkoutCart(user, cartItems, orderId, address,
    ...
    // if (!is_collect)
    //   values.order_time.format('HH:mm') > _const.EXPIRE_ADD_ORDER_TIME ?
    //     values.delivery_expire_day = moment().add(duration_days + 1, 'days').format('YYYY-MM-DD') : values.delivery_expire_day = moment().add(duration_days, 'days').format('YYYY-MM-DD');
