...
class Order extends Base {
  ...
  async calculateOrderPrice(user, orderId, cartItems) {
    let order_line_array = null;
    let order_products;
    let productIds = [];
    let productInsIds = [];
    let order;
    if (user && user.access_level === undefined) {
      // need orderId
      order = await this.OrderModel.findOne({_id: mongoose.Types.ObjectId(orderId)}).lean();
      order_line_array = order.order_lines;
    } else {
      // need cartItems -> if guest
      order_line_array = cartItems;
    }

    order_line_array.forEach(el => {
      productIds.push(mongoose.Types.ObjectId(el.product_id));
      productInsIds.push(mongoose.Types.ObjectId(el.product_instance_id));
    });
    order_products = await (new ProductModel()).findOrderProductsInfo(productIds, productInsIds);
    order_products.forEach(item => {
      const now = moment();
      item.instances.forEach(ins => {
        let quantity = order_line_array.filter(ola => ola.product_instance_id.toString() === ins._id.toString());
        ins.quantity = quantity[0].number ? quantity[0].number : quantity.length;
      });
      if (item.campaignInfo && item.campaignInfo.length) {
        item.campaignInfo = item.campaignInfo.filter(camp => (moment(camp.start_date).isSameOrBefore(now)) && (moment(camp.end_date).isSameOrAfter(now)));
        if (item.campaignInfo && item.campaignInfo.length) {
          let max_discount_ref = Math.max(...item.campaignInfo.map(el => el.discount_ref));
          item.campaignInfo = item.campaignInfo.filter(el => el.discount_ref === max_discount_ref);
        }
      }
    });
  ...
  async calculateTotalAmount(user, order_id, cartItems) {
    let info = await this.calculateOrderPrice(user, order_id, cartItems);
    let order_products_info = info['order_products'];
    let coupon_code = info['coupon_code'];
    let original_price_without_discount = 0;
    let order_goods_price = 0;
    let total_amount = 0;

    // calculate campaigns discounts
    order_products_info.forEach(item => {
      let ins_goods_price = 0;
      let camp_discount = (item.campaignInfo && item.campaignInfo.length) ? item.campaignInfo[0].discount_ref / 100 : 0;
      item.instances.forEach(ins => {
        ins_goods_price += ins.price * ins.quantity;
      });
      original_price_without_discount += ins_goods_price;
      order_goods_price += ins_goods_price - ins_goods_price * camp_discount;
    });
    total_amount = order_goods_price;

    // calculate coupon code discounts
    try {
      let coup = await this.checkCouponValidation(user, {
        coupon_code,
        product_ids: info['productIds'].map(el => el.toString()),
      });
      coup = coup[0];

      let discount = coup.discount_ref;
      if (discount <= 100) { // if percentage
        discount = original_price_without_discount * discount / 100; // apply discount on original price
        // discount = order_goods_price * discount / 100;            // apply discount on discounted price
      }
      total_amount -= discount;
    } catch (e) {
      // no coupon code or invalid/expired one
    }

    return {total_amount, order_products_info};
  }

  async prepareDataForBankGateway(user, data) {
    // calculate order price / delivery price
    let delivery_price = 0;
    let delivery_discount = 0;
    let durationInfo;

    let {total_amount, order_products_info} = await this.calculateTotalAmount(user, data.order_id, data.cartItems);
    // now need to calc delivery price with its probable discount
    // using delivery duration days received from client (data.duration_days)


    if (Math.abs((data.total_amount - data.discount) - total_amount) > 2000) {
      return Promise.reject(error.invalidInvoice);
    }

    if (data.is_collect || total_amount >= env.free_delivery_amount) { //delivery is free in orders with amount more than 5000 or in C&C mode
      delivery_price = 0;
      delivery_discount = 0;
    } else {
      // calculate delivery price and delivery discount
      // no need to this below request if duration_id comes from client instead of duration days
      durationInfo = await (new DeliveryDurationInfoModel()).getDurationInfoByDays(data.duration_days);
      let body = {
        customer_id: user ? user._id : null,
        duration_id: durationInfo._id
      };
      let resObj = await (new DeliveryDurationInfoModel()).calculateDeliveryDiscount(body);
      delivery_price = resObj.delivery_cost;
      delivery_discount = resObj.delivery_discount;
    }
    //calculate delivery price and discount correctly, now probable coupon_code will change total amount(if any coupon_code exists)
    total_amount = total_amount + delivery_price - delivery_discount;
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        this.checkoutCart(user, order_products_info, invoiceNumber, invoiceDate, data.cartItems, data.order_id, data.address, data.transaction_id, data.used_point,
          data.used_balance, total_amount, data.is_collect, data.discount, data.duration_days, data.time_slot, data.paymentType, data.loyalty)
          .then(res => {
            resolve(bankDataObj);
          }).catch(err => {
  ...
  readPayResult(user, data) {
    ...
    return new Promise((resolve, reject) => { // read transaction result
      ...
      this.OrderModel.aggregate([
        ...
        .then(() => {
          if (notPayedOrder[0].total_amount.toString() !== xmlToNodeReadRes.amount[0].toString())
            return Promise.reject(error.invalidInvoice);

          notPayedOrder[0].order_lines.forEach(el => {
            let count = 0;
  ...
  checkoutCart(user, order_products_info, invoice_number, invoice_date, cartItems, orderId, address,
    ...
    if (user && user.access_level === undefined) {
      ...
      return this.earnSpentPoint(user, paymentType, total_amount, is_collect, duration_days, recievedLoyalty, discount)
        ...
        .then(res => {
          let orderObj = res;
          orderObj.order_lines.forEach(item => {
            let temp_item = order_products_info.filter(el => el._id.toString() === item.product_id.toString())[0];
            let temp_campaignInfo = temp_item.campaignInfo;
            item.campaignInfo = {
              _id: temp_campaignInfo && temp_campaignInfo.length ? temp_campaignInfo[0]._id : null,
              discount_ref: temp_campaignInfo && temp_campaignInfo.length ? temp_campaignInfo[0].discount_ref : 0,
            };
            let temp_price = temp_item.instances.filter(ins => ins._id.toString() === item.product_instance_id.toString())[0].price;
            item.product_price = temp_price;
            item.paid_price = temp_price - temp_price * (temp_campaignInfo && temp_campaignInfo.length ? temp_campaignInfo[0].discount_ref / 100 : 0);
          });
          values.order_lines = orderObj.order_lines;
          return this.OrderModel.findOneAndUpdate({
            _id: mongoose.Types.ObjectId(orderObj._id)
          }, {
              $set: values,
            }, {new: true}).lean()
        })
      ...
      cartItems.forEach(el => {
        let temp = order_products_info.filter(op => op._id.toString() === el.product_id.toString());
        let temp2 = temp[0].instances.filter(ins => ins._id.toString() === el.product_instance_id.toString());
        el.campaignInfo = {
          _id: (temp && temp.length && temp[0].campaignInfo && temp[0].campaignInfo.length) ? temp[0].campaignInfo[0]._id : null,
          discount_ref: (temp && temp.length && temp[0].campaignInfo && temp[0].campaignInfo.length) ? temp[0].campaignInfo[0].discount_ref : 0,
        };
        el.price = temp2[0].price;
      });
    ...
    return new Promise((resolve, reject) => {
      this.getCartItems(user)
        ...
        .then(res => {
          if (res.length)
            return Promise.reject(error.invalidExpiredCouponCode);
          return models()['Campaign' + (Order.test ? 'Test' : '')].find({
            'coupon_code': body.coupon_code,
  ...
  checkoutCartDemo(user, cartItems, orderId, address,
    ...
    const values = {
      // just a mock transaction so that the whole operation doesn't fail
      transaction_id: 'xyz' + Math.floor(Math.random() * 100000),
      used_point,
      used_balance,
      address,
