...
class Order extends Base {
  ...
  async readPayResult(IPGRecData) {
    try {
      ...
      if (parsedResult || parsedResult.resultObj)
        throw new Error(`cannot parse IPG check transaction result of ${res.body}. the parsed result is: ${JSON.stringify(parsedResult, null, 2)}`)
  ...
  async verifyPayment(user, data, xmlToNodeReadRes, orderId) {
    try {
      ...
      let paresedResult = await parseString(res.body);
      ...
      let xmlToNodeVerifyRes = paresedResult.actionResult;
