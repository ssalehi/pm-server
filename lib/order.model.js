...
class Order extends Base {
  ...
  readPayResult(user, data) {
    ...
    return new Promise(async (resolve, reject) => { // read transaction result
      ...
      this.OrderModel.findOne(
        ...
        .then(async res => {
          ...
          if (xmlToNodeReadRes.result[0] === 'True') {  // transaction is unsuccessful (result: true), need to final checkout
            // TODO check repeated factors
            if (xmlToNodeReadRes.terminalCode[0] === env.terminal_code.toString() &&
              xmlToNodeReadRes.merchantCode[0] === env.merchant_code.toString() &&
              xmlToNodeReadRes.action[0] === notPayedOrder.IPG_data.action.toString() &&
              // moment(xmlToNodeReadRes.invoiceDate[0]).toDate() === notPayedOrder.IPG_data.invoice_date
              xmlToNodeReadRes.invoiceDate[0].toString() === data.invoiceDate &&  // should check in correct manner
              xmlToNodeReadRes.invoiceNumber[0].toString() === notPayedOrder.IPG_data.invoice_number.toString()) {
              //should final check for last time
              return this.OrderModel.aggregate([
                {
                  $match: {invoice_number: data.invoiceNumber, is_cart: true}
                },
                {
                  $unwind: {
                    path: '$order_lines',
        ...
        .then(res => {
          ...
          const productInsIds = [];
          orderObj = res;
          if (notPayedOrder.total_amount.toString() !== xmlToNodeReadRes.amount[0])
            return Promise.reject(error.invalidInvoice);
          ...
          orderObj.forEach(el => {
            ...
            productInsIds.push(el.instance_id);
          });

          return (new ProductModel()).productCountAfterPayment(productIds, productInsIds);
        })
        .then(res => {
          res.forEach(el => {
            cartItems.filter(item => item.product_instance_id.toString() === el.instances._id.toString())[0].count = this.remainProductCount(el);
          });
  ...
  remainProductCount(item) {
    let remainedCount = 0;
    item.instances.inventory.forEach(el => {
      remainedCount += el.count - el.reserved;
    })
