...
class Order extends Base {
  ...
  async calculateOrderPrice(user, orderId, cartItems) {
    // TODO  : campains and discounts have not been considered
    ...
    if (user && user.access_level === undefined) {
      // need orderId
      let order = await this.OrderModel.findOne({_id: mongoose.Types.ObjectId(orderId)}).lean();
  ...
  async prepareDataForBankGateway(user, data) {
    ...
    let order_products_info = await this.calculateOrderPrice(user, data.order_id, data.cartItems);
    ...
    // order goods price calculated correctly(probable coupon code will influence on order_goods_price),
    ...
    if (Math.abs((data.total_amount - data.discount) - order_goods_price) > 2000) {
