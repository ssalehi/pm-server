...
class Order extends Base {
  ...
  async checkout(user, body, demo) {
    try {
      ...
      if (!user && body.coupon_code)
        throw new Error('using coupon code is only allowd for logged in users');

      if ((!body.cartItems || !body.cartItems.length) && !body.order_id)
        throw new Error('order items cannot be found');

      if (!body.is_collect && !body.address)
        throw new Error('address must be declared for not cc order');
      ...
      if (!body.paymentType)
        throw new Error('order payment type is not declared');
  ...
  async calculateOrderPrice(orderLines, user, coupon_code) {
    ...
    if (coupon_code) {
      ...
      if (foundCouponCampaign) {
        let discount = Math.round(totalProductPrice * foundCouponCampaign.discount_ref / (100 * env.rounding_factor)) * env.rounding_factor;
  ...
  async calculateSeperatedPrice(order, orderLine) {
    try {
