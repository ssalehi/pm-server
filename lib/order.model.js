...
class Order extends Base {
  ...
  prepareDataForBankGateway(user, data) {
    // preparing data (mainly generate digital sign), to send to client form , and then send to bank gatewaye therefrom
    data.total_amount = 1000;
    const time = moment().format("YYYY/MM/DD HH:mm:ss").toString();
    const redirectAddress = "http://localhost:4200/shopResult";
        ...
        .then(res => {
          ...
          if (xmlToNodeReadRes.resultObj.result[0] == 'True') {
            if (xmlToNodeReadRes.resultObj.terminalCode[0] === env.terminal_code.toString() &&
              ...
              xmlToNodeReadRes.resultObj.action[0] === '1003' &&
  ...
  checkoutCart(user, invoiceNumber, invoiceDate, cartItems, orderId, address,
