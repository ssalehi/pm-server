...
class DeliveryTicket extends Base {
  ...
  shelvesList(receiverId, offset, limit, body) {
    ...
    if (body.hasOwnProperty('transferee') && body.transferee) {
      nameMatching.push({
        $or: [
          {$and: [{'to.customer._id': {$exists: true}}, {'customer_name': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},
          {$and: [{'to.customer.address_id': {$exists: true}}, {'recipient_name': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},
    ...
    result.mainQuery = [
      ...
      },
      {
        $project: {
          ...
          recipient_name: {$toLower: {$concat: ['$customer.recipient_name' + ' ' + '$customer.recipient_surname']}},
