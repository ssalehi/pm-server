...
class DeliveryTicket extends Base {
  ...
  getDeliveryItems(user, offset, limit, body) {
    ...
    conditions =    [{'from.warehouse_id': {$exists: false}} ];
    // conditions = [ {'is_return': false}];
    //   conditions = [{'is_return': false}];
    //  else return Promise.reject(errors.noAccess);
    ...
    //   conditions = conditions.concat([{'end': {$lte: new Date(moment(body.delivery_end).format('YYYY-MM-DD'))}}]);
    // }
    //
    // if (body.hasOwnProperty('delivery_start') && body.delivery_start) {
    //   conditions = conditions.concat([{'end': {$lte: new Date(moment(body.delivery_start).format('YYYY-MM-DD'))}}]);
    // }
    console.log('body',body);
    console.log('delivery end', body.delivery_end);
