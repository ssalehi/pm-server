...
class DeliveryTicket extends Base {
  ...
  getDeliveryItems(user, offset, limit, body) {
    ...
    if (user.access_level === _const.ACCESS_LEVEL.SalesManager) {
      ...
      // conditions.push({
      //
      //     // {$and: [{'to.warehouse_id': {$exists: true}}]},
      //     // {$and: [{'is_return': false}]},
      // })
    }
    ...
    if (user.access_level === _const.ACCESS_LEVEL.HubClerk) {
      conditions.push({
        $or: [
          ...
          {$and: [{'to.customer._id': {$exists: true}}]},
        ]
      })
    }
    ...
    if (body.hasOwnProperty('isInternal')) {
      if (body.isInternal) {
        DeliveryMatching = [{'from.warehouse_id': {$exists: true}}, {'to.warehouse._id': {$exists: true}}]
      }
      else if (body.isInternal === false) {
        DeliveryMatching.push({
          $or: [
            {$and: [{'from.warehouse_id': {$exists: true}}, {'to.customer._id': {$exists: true}}]},
            {$and: [{'to.warehouse._id': {$exists: true}}, {'from.customer._id': {$exists: true}}]},
          ]
        });
      }
    ...
    // Time matching
    let timeMatching = [];
    if (body.hasOwnProperty('startDateSearch') && body.startDateSearch) {
      let date1 = moment(moment(body.startDateSearch).format('YYYY-MM-DD'), 'YYYY-MM-DD').toDate();
      let date2 = moment(moment(body.startDateSearch).add(1, 'd').format('YYYY-MM-DD'), 'YYYY-MM-DD').toDate();
      timeMatching.push({
        $and: [
    ...
    result.mainQuery = [
      ...
      // },
      // {
      //   $addFields: {'from_adr_id_cmp': {$cmp: ['$from.customer.address._id', '$from_customer.addresses._id']}}
      // },
      // {
      //   $match: {
      //     from_adr_id_cmp: 0,
      ...
      //       'product_color_id': '$product.instances.product_color_id'
      //     },
      //     to_customer_name: {$toLower: {$concat: ['$from.customer.first_name', ' ', '$from.customer.surname']}},
      //     agent_name: {$toLower: {$concat: ['$delivery_agent.first_name', ' ', '$delivery_agent.surname']}},
      //     to_warehouse_name: {$toLower: '$from.warehouse.name'},
      //     cmp_value1: {$cmp: ['$order.order_lines._id', '$order_details.order_line_ids']},
      //     cmp_value2: {$cmp: ['$order.order_lines.product_instance_id', '$product.instances._id']},
      //     sender_agent: 1,
      ...
      //     },
      //     address: {
      //       '_id': '$from_customer.address._id',
      //       'recipient_name': '$from_customer.address.recipient_name',
      //       'recipient_surname': '$from_customer.address.recipient_surname',
      //       'recipient_national_id': '$from_customer.address.recipient_national_id'
      //     },
      //     product_colors: '$product.colors',
      //     instance: {
      //       '_id': '$product.instances._id',
      ...
      // },
      // {
      //   $addFields: {'to_adr_id_cmp': {$cmp: ['$to.customer.address._id', '$to_customer.addresses._id']}}
      // },
      // {
      //   $match: {
      //     to_adr_id_cmp: 0,
