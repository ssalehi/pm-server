    ...
    if (body.hasOwnProperty('transferee') && body.transferee) {
      nameMatching.push({
        $or: [
          ...
          {$and: [{'order.address._id': {$exists: true}}, {'recipient_name': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},

        ]
      });
      console.log('transferee', body.transferee);
    }
    ...
    result.mainQuery = [
      ...
      },
      {
        $project: {
          ...
          recipient_name: {$toLower: {$concat: ['$order.address.recipient_name' + ' ' + '$order.address.recipient_surname']}},
