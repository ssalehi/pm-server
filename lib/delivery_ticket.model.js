...
class DeliveryTicket extends Base {
  ...
  getDeliveryItems(user, offset, limit, body) {
    ...
    if (user.access_level === _const.ACCESS_LEVEL.SalesManager) {
      conditions.push({
        $or: [
          {$and: [{'from.warehouse_id': {$exists: true}}]},
    ...
    if (body.hasOwnProperty('sender') && body.sender) {
      nameMatching.push({
        $or: [
    ...
    result.countQuery = [
      ...
      },
      {
        $project: {
          ...
          transaction_id: '$order.transaction_id',
          order_time: '$order.order_time',
          from_customer_name: {$toLower: {$concat: ['$from_customer.first_name', ' ', '$from.customer_surname']}},
          from_warehouse_name: {$toLower: '$from.warehouse.name'},
          to_customer_name: 1,
          is_return: 1,
          start: 1,
          end: 1,
      ...
      },
      {
        $group: {
          ...
          order_time: {$first: '$order_time'},
          from_customer_name:  {$first: '$customer_name'},
          from_warehouse_name: {$first: '$from_warehouse.name'},
          count: {
            $sum: 1
          },
        }
