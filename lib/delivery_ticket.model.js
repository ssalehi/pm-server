...
class DeliveryTicket extends Base {
  ...
  getDeliveryItems(user, offset, limit, body) {
    ...
    if (body.recipient) {
      ...
      nameMatching.push({'to.customer': {$exists: true}}, {'to_customer_': new RegExp('.*' + body.recipient.toLowerCase() + '.*')});
    }
    ...
    if (body.transferee) {
      nameMatching.push({
        $or: [
          ...
          {$and: [{'to.customer._id': {$eq: null}}, {'to_recipient_name': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},
          {$and: [{'to.customer._id': {$eq: null}}, {'to_recipient_national_id': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},
    ...
    let sortOption = {};
    if (body.senderColumn && body.receiverColumn && body.direction) {
      const direction = body.direction === 'asc' ? 1 : -1;

      if (body.senderColumn === 'name' && body.receiverColumn === 'name') {
        sortOption = {
        ...
        sortOption[body.senderColumn] = direction;
        sortOption[body.receiverColumn] = direction;
