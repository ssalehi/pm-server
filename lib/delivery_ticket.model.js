  ...
  getDeliveryItems(user, offset, limit, body) {
    ...
    if (body.isInternal) {
      ...
      DeliveryMatching.push({
        $or: [
          {$and: [{'to.customer.address': {$exists: true}}]},
          {$and: [{'from.customer._id': {$exists: true}}]},
        ]
      });
    }
    ...
    if (body.recipient) {
      nameMatching.push({
          $or: [
            {$and: [{'toRecipientName': new RegExp('.*' + body.recipient.toLowerCase() + '.*')}]},
            {$and: [{'toCustomerName': new RegExp('.*' + body.recipient.toLowerCase() + '.*')}]}
          ]
        }
      );
    }
    ...
    if (body.transferee) {
      nameMatching.push({
        $or: [
          {$and: [{'to.customer': {$exists: true}}, {'to_recipient': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},
          {$and: [{'to.customer': {$exists: true}}, {'to_recipient_national_id': new RegExp('.*' + body.transferee.toLowerCase() + '.*')}]},
        ]
      });
    }
    ...
    result.mainQuery = [
      ...
      },
      {
        $project: {
          ...
          // customerName: {$concat: ['$customerName.first_name', ' ', '$customerName.surname']},
          // recipientName: {$concat: ['$order.address.recipient_name', ' ', '$order.address.recipient_name']},
          cmp_value1: {$cmp: ['$order.order_lines._id', '$order_details.order_line_ids']},
          cmp_value2: {$cmp: ['$order.order_lines.product_instance_id', '$product.instances._id']},
        },
      },
