...
class Customer extends Person {
  ...
  async generateCode() {
    ...
    if (tokenStr.IsSuccessful) {
      ...
      return Promise.reject('خطای ارسال توکن از سمت سرور');
    }
  }
    ...
    return new Promise((resolve, reject) => {
      this.generateCode()
        .then(res => {
          ...
          return this.CustomerModel.findOneAndUpdate({'username': username}, {$set: {verification_code: res.code}});
        })
        .then(res => {
  ...
  setMobileNumber(user, mobile_no) {
    ...
    return new Promise((resolve, reject) => {
      this.generateCode()
        ...
        .then(res => {
          resolve({done: 'ok', message: 'code sent to mobile numberrr'})
        })
        .catch(err => {
          reject(err);
        });
