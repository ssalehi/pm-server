...
class Customer extends Person {
  ...
  registration(body) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.CustomerModel.find({$or: [{username: body.username}, {mobile_no: body.mobile_no}]}).lean()
        .then(res => {
          console.log('====>',res);
        ...
        .then(code => {
          ...
          return regVerModel.find({
            'customer_data.username': body.username,
            'customer_data.mobile_no': body.mobile_no
          }).lean();
        })
        .then(res => {
          if (res.length > 0)
  ...
  resendVerificationCode(username) {
    ...
    return new Promise((resolve, reject) => {
      this.generateCode()
        .then(code => {
          return regVerModel.update({'customer_data.username': username}, {$set: {code: code}});
        })
