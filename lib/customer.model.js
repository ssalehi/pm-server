...
const smsSender = require('../lib/smsSender.model');
...
class Customer extends Person {
  ...
  async verification(user, code, username) {
    ...
    return {done: 'ok', message: 'code is accepted', is_verified: res.is_verified};
  }
  ...
  async resendVerificationCode(user, username) {
    try {
      ...
      const foundCostomer = await this.CustomerModel.findOneAndUpdate({'username': username}, {$set: {verification_code: res.code}});
  ...
  async sendActivationMail(email, isForgotMail) {
    ...
    try {
      ...
      let mailContent = this.composeActivationMail(updatedCustomer.first_name, updatedCustomer.surname, activation_link, isForgotMail);
