...
class Customer extends Person {
  ...
  getWishListItems(user) {
    ...
    // return this.CustomerModel.find({_id: mongoose.Types.ObjectId(user.id)})
    ...
    return this.CustomerModel.aggregate(
      ...
      if (res.length) {
        res.forEach(el => {
          el.product[0].instances.forEach(i => {
            if (i._id.toString() === el.wish_list.product_instance_id.toString()) {
              el.wishedItem = i;
            }
          })
        });
        res.forEach(el => {
          el.product[0].colors.forEach(c => {
            if (c._id.toString() === el.wishedItem.product_color_id.toString())
              el.wishedItem.wishedColor = c;
          })
        });
