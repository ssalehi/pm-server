...
class Customer extends Person {
  ...
  verification(user, code) {
    ...
    return new Promise((resolve, reject) => {
      this.CustomerModel.find({verification_code: code, username: user.username}).lean()
        .then(res => {
          if (res.length === 0)
            return Promise.reject(error.codeNotFound);

          return this.CustomerModel.findOneAndUpdate({username: user.username}, {
            $set: {
              verification_code: null,
  ...
  setMobileNumber(user, mobile_no) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.generateCode()
        .then(res => {
          newCode = res;
          return this.CustomerModel.find({username: user.username});
        })
        .then(res => {
          if (res.length < 1)
            return Promise.reject(error.noUser);
