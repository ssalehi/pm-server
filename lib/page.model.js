...
class Page extends Base {
  ...
  getPageByAddress(address, getNotFinalPlacements = false) {
    ...
    if (!getNotFinalPlacements)
      Object.assign(query, {$or: [{'placement.is_finalized': true}, {'placement.is_finalized': {$eq: null}}]});

    return new Promise((resolve, reject) => {
      this.PageModel.findOne(query).select('placement page_info')
        .then(res => {
