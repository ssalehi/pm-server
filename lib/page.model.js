...
class Page extends Base {
  ...
  getPageAllPlacement(address) {
    ...
    return this.PageModel.findOne({_id: mongoose.Types.ObjectId(id)}).select('placement is_app address');
    // return this.PageModel.findOne({}).select('placement is_app address');
  }

  getPageComponentName(id) {
    let component_name_array = [];
    if(!id) return Promise.reject(error.pageIdRequired);
    if (!mongoose.Types.ObjectId(id)) return Promise.reject(error.pageIdIsNotValid);

    return this.PageModel.findOne({_id: mongoose.Types.ObjectId(id)}).select('placement')
      .then(res => {
        console.log(res.placement.length);
        component_name_array = res.placement.map(el => el.component_name);
        console.log('component_name_array : ', component_name_array);
        Promise.resolve(component_name_array);
    });
  }
