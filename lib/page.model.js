      ...
      .then(res => {
        if (!res) {
          ...
          if (getNotFinalPlacements) {
            ...
            const spDate = body.date ? body.date : moment().format('YYYY-MM-DD');
            ...
            return models['ArchivePlacement' + (Page.test ? 'Test' : '')].find({page_id: res._id})
              .then(archive_list => {
                archive_list = archive_list.filter(el => moment(moment(el.start_date).format('YYYY-MM-DD')).isSameOrBefore(spDate) && 
    ...
    return new Promise((resolve, reject) => {
      this.model.findOne({_id: mongoose.Types.ObjectId(body.page_id)}).lean()
        .then(res => {
          ...
          return this.archivePlacement(body.page_id, archiveList);
        })
        .then(res => {
