...
class Page extends Base {
  ...
  addPlacement(body) {
    ...
    const prc = placementsRequirementsConstraints.find(
      el => el.component_name.toLowerCase() === body.placement.component_name.toLowerCase() &&
        (!el.variable_name || el.variable_name.toLowerCase() === body.placement.variable_name.toLowerCase()));

    prc.required.forEach(el => {
      if (!body.placement.info[el]) {
        isIncomplete = true;
        return;
      }
    });
    ...
    return new Promise((resolve, reject) => {
      // Check placement has image or not
      const newId = mongoose.Types.ObjectId();

      const imgPath = env.uploadPlacementImagePath + (Page.test ? path.sep + 'test' : '')
        + path.sep + body.page_id + path.sep + newId;

      (body.placement.info.imgUrl ? this.moveImage(imgPath, body.placement.info.imgUrl) : Promise.resolve(false))
        .then(res => {
          if (res)
            body.placement.info.imgUrl = imgPath;
          ...
          return (Object.keys(constraints).length > 0 ? this.model.find(constraints) : Promise.resolve([]))
        })
        .then(res => {
          if (res.length > 0)
  ...
  //   Object.assign(body.info, { imgUrl: path });
  //   return this.addPlacement(Object.assign(body, { page_id: params.page_id }))
  //     .then(res => {
  //       return Promise.resolve(Object.assign(res, { downloadURL: path }));
  //     });
  // }
      ...
      this.model.find({ _id: pageId }).lean()
        .then(res => {
          ...
          if (placementObj.is_finalized && !placementObj.ref_newest_id) {
            ...
            const imgPath = env.uploadPlacementImagePath + (Page.test ? path.sep + 'test' : '')
              + path.sep + pageId + path.sep + data._id;

            let newPlacementObj;

            return (data.info.imgUrl ? this.moveImage(data.info.imgUrl, imgPath) : Promise.resolve(false))
              .then(res => {
                if (res)
                  data.info.imgUrl = imgPath;
              ...
              .then(res => {
                return this.model.findOneAndUpdate({
                  ...
                  'placement._id': mongoose.Types.ObjectId(placementObj._id),
                    ...
                    $set: {
                      'placement.$.ref_newest_id': newPlacementObj._id,
                    }
                  }, {
                    new: true,
                  });
              ...
              .then(res => {
                return Promise.resolve(Object.assign(res._doc, {
                  new_placement: newPlacementObj,
                  placement_id: newPlacementObj._id,
                }));
              })
  ...
  moveImage(currentPath, destination_path) {
    return new Promise((resolve, reject) => {
      fs.rename(currentPath, destination_path, (err) => {
        if (err)
          reject(err);
        else
          resolve(true);
