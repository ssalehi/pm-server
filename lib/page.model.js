...
class Page extends Base {
  ...
  getPageAllPlacement(address) {
    ...
    return this.PageModel.findOne({address : address}).select('placement is_app address');
  }

  getPagePlacementBrief(id, offset, limit) {
    ...
    return this.PageModel.findOne({_id: mongoose.Types.ObjectId(id)}).select('placement.component_name').skip(offset).limit(limit);
  }
  ...
  suggest(phrase, options) {
    ...
    let is_app = options.is_app ? options.is_app : false;
    ...
    return this.PageModel.find({
      ...
      'is_app': is_app
    }).limit(5).sort({address: 1}).select('address');
  }
