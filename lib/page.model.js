...
class Page extends Base {
  ...
  setPage(body, id) {
    // should check if a page with this. collection id is exist or not
    if (!body.address)
    ...
    if (!id) {
      return this.PageModel.findOne({'page_info.collection_id': body.collection_id}).lean()
        .then(res => {
          if(!res) {
            let obj = {
    ...
    return this.PageModel.findOne({address: body.address}).select('placement page_info').lean()
      .then(res => {
        if (!res) {
          ...
          // let finalList = res.placement.filter(el => el.is_finalized === true);
          if (getNotFinalPlacements) {
            // Should get placements archive in archivePlacement collection
  ...
  suggest(phrase, options) {
    return this.PageModel.find({
      $and: [{address: {$regex: phrase, $options: 'i'}},
      {address: {$ne: options.exceptionAddress}}]
    }, {address: 1})
  ...
  addPlacement(body) {
    ...
    return new Promise((resolve, reject) => {
      ...
      (constraints.length > 0 ? this.model.aggregate([
        ...
        {
          $project: {
            'placement.component_name': 1,
            'placement.variable_name': 1,
            'placement.info': {
              $cond: {
                if:
                {
