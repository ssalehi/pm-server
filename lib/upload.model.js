...
Dictionary = {
  ...
  // -> says whether to override the already counts of products with this new counts
  // Notes:
  //  if set to true and the new count is 0, product will also be added to soldout table
  Override: {excelName: 'Override'},

  // -> says whether to set the soldout mark in the soldout table or not (if exists)
  // Notes:
  ...
  excel(file) {
    ...
    return this.safeInsert(brands, 'Brand')
      ...
      .then(res => {
        ...
        return new WarehouseModel(Upload.test).getWarehouses();
      })
      .then(res => {
        ...
        let makeProduct = row => {
          ...
          return models()['Product' + (Upload.test ? 'Test' : '')].findOneAndUpdate(query, update, options).lean()
            .then(res => {
              ...
              console.log('-> ', `${res.article_no} | ${counter} of ${total}`);
            ...
            .then(res => {
              ...
              let setInstanceInventory = (instance) => {
                ...
                warehouses.forEach(w => {
                  ...
                  if (foundInventory) {
                    // read override flag to see if the counts should be updated or set explicitly
                    let override = row[Dictionary.Override.excelName] && row[Dictionary.Override.excelName].toUpperCase() || falses[0];
                    if (trues.some(el => el === override))
              ...
              // check if product is already soldout and is added again, should be removed from soldout list
              let checkSoldOut = function () {
                let totalProductCount = foundInstance.inventory.map(i => i.count).reduce((a, b) => a + b, 0) || 0;

                if (totalProductCount <= 0) {
                  // count is 0, so must be added to soldout list (if not already)
                  return new SoldOutModel(Upload.test).insertProductInstance(productId.toString(), foundInstance._id.toString()).catch(err => Promise.resolve(err))
                    .then(res => {
                      let soldOutFlag = row[Dictionary.SoldOut.excelName] && row[Dictionary.SoldOut.excelName].toUpperCase() || falses[0];
                      if (trues.some(el => el === soldOutFlag)) {
                        // set the flag if the soldout flag in excel was set to true
                        foundInstance.sold_out = true;
                        return new SoldOutModel(Upload.test).setSoldOutFlagOnPI({
                          productId: productId.toString(),
                          productInstanceId: foundInstance._id.toString(),
                          soldOutStatus: true
                        });
                      } else {
                        // do nothing
                ...
                return new SoldOutModel(Upload.test).removeProductInstance(productId.toString(), foundInstance._id.toString()).catch(err => Promise.resolve());
              };

              checkSoldOut().then(res => {
                let query = {
                  _id: productId,
        ...
        return data.map(row => () => makeProduct(row)).reduce((x, y) => {
          return x.then(y).catch(err => console.error("Excel Error: ", err))
        }, Promise.resolve());
      }
      )
