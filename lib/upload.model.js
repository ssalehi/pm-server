  ...
  excel(file) {
    ...
    function createTags(array) {
      return new Promise((resolve, reject) => {
        ...
        Object.keys(dataList).forEach((key) => {
          promiseList.push(models['TagGroup' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
            name: key,
          }, {$set: {name: key}}, {upsert: true, 'new': true}).then(group => {
            let innerPromiseList = [];
            dataList[key].forEach(value => {
              innerPromiseList.push(models['Tag' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
    ...
    function createColors(array) {
      return new Promise((resolve, reject) => {
        ...
        array.forEach((item) => {
          promiseList.push(
            models['Color' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
    ...
    function renameObject(array) {
      let list = [];
      let itemsRename = ['color_code', 'product_name', 'size', 'qty', 'price', 'uom', 'color_description', 'product_type', 'tags', 'subDivision'];
      ...
      for (let item in array) {
        ...
        itemsRename.forEach((el, i) => {
          if (i === 0) {
            ...
            let arr = [];
            innerArr.slice(8, innerArr.length).forEach(inner_el => {
              arr.push(inner_el);
            });
            objJson[el] = arr;
          } else {
    ...
    function createProductTypes(array) {
      return new Promise((resolve, reject) => {
        ...
        newArr.forEach((item, index) => {
          promiseList.push(models['ProductType' + (Upload.test ? 'Test' : '')].findOneAndUpdate({name: item},
    ...
    function createProduct(array) {
      ...
      // brand
      models['Brand' + (Upload.test ? 'Test' : '')].findOneAndUpdate({name: 'Nike'}, {name: 'Nike'}, {
        upsert: true,
        'new': true
      }).then(brandId => {
        ...
        array.forEach((product, index) => {
          ...
          return models['Color' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
            ...
            .then(color => {
              _color = color;
              //product color
              return models['ProductColor' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
                  color_id: color._id
                },
                {color_id: color._id},
                {upsert: true, 'new': true});
            ...
            .then(pColor => {
              _pColor = pColor;
              //product types
              return models['ProductType' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
            ...
            .then(pType => {
              ...
              product.tags.forEach(tag => {
                promises.push(models['Tag' + (Upload.test ? 'Test' : '')].findOneAndUpdate({name: tag}, {name: tag}, {
                  upsert: true,
                  'new': true
                }));
            ...
            .then(tags => {
              ...
              tags.forEach(tag => {
                models['Product' + (Upload.test ? 'Test' : '')].find({
                  ...
                  'instances.product_color_id': _color._id,
                    ...
                    if (!res || res.length === 0) {
                      ...
                      models['Product' + (Upload.test ? 'Test' : '')].findOneAndUpdate({
                          name: product.product_name,
                          product_type: _pType._id,
                          ...
                          $addToSet: {
                            ...
                            'instances': {
                              ...
                              inventory: {
                                ...
                                count: product.qty && Number(product.qty)
                              },
                            }
                          },
                          // $push: {'tags': tag._id}
