    ...
    return this.insertBrands()
      .then(res => {
        ...
        let color_names = Array.from(new Set(data.map(x => {
          return x[Dictionary.ColorDescription.excelName] || unknownColor
        })));

        color_names = color_names.map(name => {  // some products have no color description in excel file
          return {name}
        });
                ...
                $set:
                  {
                    ...
                    "product_type": {name: row[Dictionary.Division.excelName], product_type_id: typeId},
                    "brand": {name: results.brands.name, brand_id: brandId},
                    "base_price": 0,
                    "date": new Date(),
                    "campaigns": []
                  }
            ...
            return models['Product' + (Upload.test ? 'Test' : '')].findOneAndUpdate(query, update, options)
              .then(res => {
                ...
                let findTagId = tagGroupName => {
                  ...
                  let tags = results.tags.filter(x => x.name === row[this.mapName(tagGroupName, true)] && x.tag_group_id.equals(tgId));
                  return tags[0]._id;
