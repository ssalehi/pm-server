...
class Upload extends Base {
  ...
  /*
  == the inventories of product instances should be updated (the count, to be specific),
  not a new element for that warehouse be added to the array!

    add two boolean columns to the excel, as 'override' and 'soldout':

    - the override says that the inventories are updated to the number in the excel
    e.g. if we already have 2 products and we in the excel, we have 3 products with override=false,
    it will become 5, but if it's override=true, it's gonna be 2 products
    if he sets the products number to 0, then the product number is set to 0 and also added to soldout

    - the soldout says that whether to set the soldout mark in the soldout table or not (if exists!)

    NOTE: edge cases are not yet considered. :-?
  */
