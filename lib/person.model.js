...
class Person extends Base {
  ...
  loginCheck(username = this.username, password = this.password, personType = PersonType.Customer, access_level) {
    return new Promise((resolve, reject) => {
      this.load(username, password, personType, access_level)
  ...
  load(username, password, personType, access_level) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let searchData = {username: username, active: true};
      ...
      curCon.findOne(searchData).lean()
        .then(res => {
          if (res) {
  ...
  static deserialize(req, person, done) {
    ...
    let instance = new Person(personType, req.app.get('env') === 'development' ? req.query.test === 'tEsT' : false);
    instance.userCheck(personType, person.id)
      .then((foundPerson) => {
  ...
  static afterLogin(user, isAgent) {
    ...
    return new Promise((resolve, reject) => {
      // Person.setNamespace(username);
      curCon.findOne({_id: mongoose.Types.ObjectId(user._id), active: true}).lean()
        .then(res => {
          if (!res)
            reject(error.noUser);
          else {
            if (res.hasOwnProperty('is_verified') && !res.is_verified)
              reject(error.notVerified);
