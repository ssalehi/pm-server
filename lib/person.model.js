...
class Person extends Base {
  ...
  load(username, password, personType, access_level) {
    ...
    if (personType === PersonType.Customer)
      searchData = Object.assign({$or: [{username: { $toLower: username}}, {mobile_no: username}], is_verified: true}, searchData);
  ...
  static jwtStrategy(req, isAgent = false) {
    ...
    let condition = {username: decoded_token.username, _id: decoded_token.id};

    if (isAgent)
      condition = Object.assign(condition, {active: true});
    else
      condition = Object.assign(condition, {is_verified: true});
