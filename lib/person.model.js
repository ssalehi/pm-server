        ...
        if (!res)
          ...
          if (isAgent) {
            ...
            if (user.hasOwnProperty('warehouse_id')) {
              const token = helpers.generateToken(user.warehouse_id).then(token => {
      ...
      .then((foundPerson) => {
        ...
        if (person.googleAuth) {
          rp({
            ...
            .then(res => {
              foundPerson['id'] = foundPerson._id;
              if (person.hasOwnProperty('warehouse_id'))
  ...
  static passportLocalStrategy(req, username, password, done) {
    ...
    person.loginCheck(username, password, personType, req.body.loginType)
      .then(foundPerson => {
        ...
        if (req.body.loginType === _const.ACCESS_LEVEL.ShopClerk.toString() || req.body.loginType === _const.ACCESS_LEVEL.SalesManager.toString()) {
