...
class Person extends Base {
  ...
  load(username, password, personType, access_level) {
    ...
    if (personType === PersonType.Customer)
      searchData = Object.assign({$or: [{username: username}, {mobile_no: username}], is_verified: true}, searchData);
    if (personType === PersonType.Agent)
      searchData = Object.assign({username, access_level}, searchData);
      ...
      .then(foundPerson => {
        ...
        if (loginType && (loginType === _const.ACCESS_LEVEL.ShopClerk || loginType === _const.ACCESS_LEVEL.HubClerk)) {
          ...
          if (!req.body.hasOwnProperty('warehouse_id')) {
            error = new Error('agent warehouse is not defined');
            done(error, false);
            return;
          ...
          warehouse.getAll().then(warehouses => {
            ...
            const is_hub = req.body.warehouse_id === warehouses.find(x => x.is_hub)._id.toString();

            if (loginType === _const.ACCESS_LEVEL.HubClerk && !is_hub) {
              error = new Error('no access to this warehouse');
              done(error, false);
              return;
              // todo: test must be written
            }
            if (loginType === _const.ACCESS_LEVEL.ShopClerk && is_hub) {
