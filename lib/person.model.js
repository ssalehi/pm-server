        ...
        if (person.googleAuth) {
          rp({
            ...
            .then(res => {
              ...
              if (person.hasOwnProperty('warehouse_id'))
                foundPerson['warehouse_id'] = person._id;
              done(null,);
            })
            .catch(err => {
              done(err);
  ...
  static passportLocalStrategy(req, username, password, done) {
    ...
    person.loginCheck(username, password, personType, req.body.loginType)
      .then(foundPerson => {
        ...
        if (req.body.loginType === _const.ACCESS_LEVEL.ShopClerk.toString()) {
          if (!req.body.hasOwnProperty('warehouse_id')) {
            let error = new Error('shop clerk warehouse is not defined');
            done(error, false);
