...
class Person {
  ...
  afterLogin(user, isAgent) {
    ...
    // Person.setNamespace(username);
    return this.PersonModel.findOne({_id: mongoose.Types.ObjectId(user._id), active: true}).lean()
      .then(res => {
        if (!res)
          return Promise.reject(error.noUser);
        else {
  ...
  static deserialize(req, person, done) {
    ...
    Person.userCheck(req, person.personType, person._id)
      .then((foundPerson) => {
  ...
  static passportLocalStrategy(req, username, password, done) {
    ...
    let person = new Person(personType , Person.isTest(req));
