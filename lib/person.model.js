    ...
    return curCon.findOne(condition)
      .then(res => {
        if (!res)
          ...
          if (res.is_verified === _const.VERIFICATION.bothVerified ||
            // app google login condition -> should allow only for the setting mobile route!
            (res.is_verified === _const.VERIFICATION.emailVerified && req.url.toLowerCase().includes(setMobileRoute))) {
            req.user = {
              id: res._id,
              googleAuth: null,
              personType: PersonType.Customer,
    ...
    return curCon.findOne({username: data.username})
      .then(res => {
        if (res) {
          delete data.is_verified;
          delete data.secret;
          return curCon.findOneAndUpdate({
            _id: res._id
          }, {
            $set: data,
          }, {
