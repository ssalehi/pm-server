...
class SMMessage extends Base {
  ...
  async assignToReturn(body, user) {
    try {
      ...
      let foundDelivery = await new DeliveryModel(this.test).assignToReturn(foundedAddress, foundOrder, foundOrderLine, user.id, body.preCheck);
  ...
  searchHistory(offset, limit, body) {
    ...
    if (body.startDateSearch) {
      ...
      startTimeMatching.push({
        // $and: [
      publish_date: {$gte: date1},
          // {publish_date: {$lt: date2},
        // ]
      })
    }
    if (timeMatching.$and.includes('publish_date') && !body.startDateSearch) {
      timeMatching.$and.pop('publish_date')
    }

    if (!startTimeMatching.length)
      startTimeMatching = {};
    else
      startTimeMatching = {$and: startTimeMatching};


    let endTimeMatching = [];
    if (body.endDateSearch) {
      let date1 = moment(moment(body.endDateSearch).format('YYYY-MM-DD')).set({
        'hour': '00',
        'minute': '00',
        'second': '00'
      }).toDate();
      ...
      endTimeMatching.push({
        // $and: [
        //   {close_date: {$gte: date1}},
          close_date: {$lte: date2},
        // ]
      })
    }
    if (timeMatching.$and.includes('close_date') && !body.endDateSearch) {
      timeMatching.$and.pop('close_date')
    }

    // if (!endTimeMatching.length)
    //   endTimeMatching = {};
    // else
    //   endTimeMatching = {$and: endTimeMatching};
    // if (!timeMatching.length)
    //   timeMatching = {};


    if (!body.endDateSearch && !body.startDateSearch) {
      timeMatching = {};
    }
