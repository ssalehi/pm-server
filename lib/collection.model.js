  ...
  getProducts(collectionId, manualProducts = false) {
    ...
    return this.CollectionModel.aggregate([
      ...
      },
      {
        $group: {
          _id: '$_id',
          products_id: {$first: '$products._id'},
          name: {
            $first: '$name',
          },
          name_fa: {
      ...
      .then(res => {
        ...
        if (res.products._id !== 0 || res.tags._id !== 0 || res.types._id !== 0) {
          ...
          if (res.products._id && res.products._id.length)
            products = res.products;

          if (res.types._id && res.types._id.length && !manualProducts)
            types = res.types;

          if (res[0].tags._id && res.tags._id.length && !manualProducts)
            tags = res.tags;

          if (res.brands._id && res.brands._id.length && !manualProducts)
            brands = res.brands;

          if (products.length === 0 && types.length === 0 && tags.length === 0)
            return Promise.resolve([]);

          return (new ProductModel(Collection.test)).getProducts(products, types, tags, brands, manualProducts); // override sold out search
        }
        else
          return Promise.resolve([])
      })
