...
class Collection extends Base {
  ...
  async getProducts(collectionId, manualProducts = false) {
    ...
    try {
      ...
      let collection = await this.CollectionModel.findById(collectionId).lean();
      let products = [];
      if (collection.productIds.length !== 0 || collection.tagIds !== 0 || collection.typeIds !== 0 || collection.brandIds !== 0) {
    ...
    return this.CollectionModel.aggregate([
      ...
      .then(res => {
        ...
        if (collection.productIds && collection.productIds.length)
          productIds = collection.productIds;
          console.log('productsIDD',products.map(p => p._id));
          if (res.types && res.types.length && !manualProducts)
            types = res.types;

        if (collection.typeIds && collection.typeIds.length && !manualProducts)
          typeIds = collection.typeIds;
          if (res.tags && res.tags.length && !manualProducts)
            tags = res.tags;

        if (collection.tagIds && collection.tagIds.length && !manualProducts)
          tagIds = collection.tagIds;
          if (res.brands && res.brands.length && !manualProducts)
            brands = res.brands;

        if (collection.brandIds && collection.brandIds.length && !manualProducts)
          brandIds = collection.brandIds;

        if (productIds.length !== 0 || typeIds.length !== 0 || tagIds.length !== 0 || brandIds.length === 0)
          products = await (new ProductModel(Collection.test).getProducts(productIds, typeIds, tagIds, brandIds, manualProducts)); // override sold out search
      }
      return Promise.resolve({
        ...
        products
          ...
          return (new ProductModel(Collection.test)).getProducts(products.map(p => p._id).filter(el => el) , types.map(t => t._id).filter(el => el), tags.map(t => t._id).filter(el => el), brands.map(b => b._id).filter(el => el), manualProducts); // override sold out search
        }
        else
          return Promise.resolve([])
      })
