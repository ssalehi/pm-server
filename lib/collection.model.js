    ...
    return this.CollectionModel.aggregate([
      ...
      .then(res => {
        ...
        if (res.products._id !== 0 || res.tags._id !== 0 || res.types._id !== 0) {
          ...
          return (new ProductModel(Collection.test)).getProducts(products.map(p => p.id), types.map(t => t.id), tags.map(t => t.id), brands.map(b => b.id), manualProducts); // override sold out search
        }
        else
          return Promise.resolve([])
      })
