  ...
  async receiveResponse(body) {
    try {
      if (
        ...
        throw new Error('incomplete receive resoponse data');
      }
      ...
      else if (type === _const.RECEIVE_TYPE.Warehouse)
        return new DSS(this.test).afterReceiveWarehouse(foundOrder, foundOrderLine, body.userId, body.warehouseId);
