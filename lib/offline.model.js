...
class Offline {
  ...
  async recieveRequest(orderId, orderLineId, barcode, user, type) {
    try {
      if (this.test)
      return Promise.resolve();

    return helpers.httpPost(`http://${env.serviceAddress}/${env.serviceReceiveAPI}`, {
      orderId: orderId.toString(),
      orderLineId: orderLineId.toString(),
      userId: user.id.toString(),
      warehouseId: user.warehouse_id.toString(),
