    ...
    try {
      ...
      if (body.orderLineId && mongoose.Types.ObjectId.isValid(body.orderLineId) &&
        ...
        if ([
          ...
          return new DSS(this.test).afterReturnDamagedVerified(foundOrder, foundOrderLine, body.userId, body.warehouseId);
        }
        else if ([
          _const.ORDER_LINE_STATUS.WaitForLost,
          ...
          return new DSS(this.test).afterRetuenLostVerified(foundOrder, foundOrderLine, body.userId, body.warehouseId);
        }
        else if ([
          _const.ORDER_LINE_STATUS.WaitForReturn,
          _const.ORDER_LINE_STATUS.ReturnVerified,
        ].includes(lastTicket.status)) {
          return new DSS(this.test).afterRetuenVerified(foundOrder, foundOrderLine, body.userId);
        }
      }
      else
        throw new Error('offline system error: invalid receive data');
