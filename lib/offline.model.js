...
class Offline {
  ...
  getTransferId(type) {
    ...
    switch (type) {
      ...
      case _const.TRANSFER_TYPE.OnlineToWarehouse:
        return 2;
      case _const.TRANSFER_TYPE.WarehouseToDamageLost:
        return 3;
      case _const.TRANSFER_TYPE.OnlineToDamageLost:
        return 4;
      default:
  ...
  getTransferType(dest) {
    switch (dest) {
      case 1:
        return _const.TRANSFER_TYPE.WarehouseToOnline;
      case 2:
        return _const.TRANSFER_TYPE.OnlineToWarehouse;
      case 3:
        return _const.TRANSFER_TYPE.WarehouseToDamageLost;
      case 4:
        return _const.TRANSFER_TYPE.OnlineToDamageLost;
      default:
  ...
  async transferRequest(orderId, orderLineId, barcode, user, type) {
    try {
      ...
      let transferId = this.getTransferData(type);
  ...
  async transferResponse(body) {
    try {
      ...
      let type = this.getTransferType(body.transferId);
      let func;

      if (type === _const.TRANSFER_TYPE.WarehouseToOnline)
        return new DSS(this.test).afterWarehouseToOnlineTransfer(body.orderId, body.orderLineId, body.userId, body.warehouseId);
      else if (type === _const.TRANSFER_TYPE.OnlineToWarehouse)
        return new DSS(this.test).afterOnlineToWarehouseTransfer(body.orderId, body.orderLineId, body.userId, body.warehouseId);
      else if (type === _const.TRANSFER_TYPE.OnlineToDamageLost || type === _const.TRANSFER_TYPE.WarehouseToDamageLost)
        return new DSS(this.test).afterToDamageLostTransfer(body.orderId, body.orderLineId, body.warehouseId);
  ...
  // async requestOnlineWarehouse(orderId, orderLineId, barcode, user, type) {
  // }
