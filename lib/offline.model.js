...
class Offline {
  ...
  getTransferData(type, warehouseId) {

    let orig = warehouseId;
    let dest = 1;

    if (!orig || dest)
      throw new Error('cannot find orig or dest of transfer');
  ...
  async transferRequest(orderId, orderLineId, barcode, user, type) {
    try {
      ...
      let {orig, dest} = this.getTransferData(type, user.warehouse_id);
  ...
  async requestLostWarehouse(orderId, orderLineId, barcode, user, reverse = false) {
    try {
