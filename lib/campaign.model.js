...
class Campaign extends Base {
  ...
  setCampaign(body, campaignId) {
    ...
    if (campaignId) {
      ...
      return this.removeCollections(campaignId)
        .then(res => {
      ...
      return new this.CampaignModel(newInfo.$set).save();
    }
  }
  ...
  removeCollections(campaignId) {
   ...
   return this.CampaignModel.findById(mongoose.Types.ObjectId(campaignId)).lean()
      .then(res => {
        ...
        if (res.collection_ids && res.collection_ids.length) {

          const promises = [];
          res.collection_ids.forEach(x => {
            promises.push(this.removeCollection(campaignId, x));
          })
          return Promise.all(promises);
        } else
  ...
  removeCampaign(cid) {
    this.removeCollections(cid).then(() => {
