...
class Campaign extends Base {
  ...
  setCampaign(body) {
    ...
    if (body.campaignId)
      return this.CampaignModel.update({
        _id: mongoose.Types.ObjectId(body.campaignId)
      }, newInfo);
    else
  ...
  getActiveCampaignByCoupon(couponCode) {
  ...
  calculateCampaignDiscount(basePrice, discount_ref) {
    try {

      if (discount_ref < 100)
        discount = Math.round(basePrice * foundCouponCampaign.discount_ref / (100 * env.rounding_factor)) * env.rounding_factor;
      else
        discount = Math.round(discount_ref / env.rounding_factor) * env.rounding_factor;
    } catch (err) {
      console.log('-> error on calclute campaign discount', err);
      throw err;
    }
