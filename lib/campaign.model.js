...
class Campaign extends Base {
  ...
  calculateCampaignDiscount(basePrice, discount_ref) {
    try {
      ...
      if (discount_ref < 100)
        return Math.round(basePrice * discount_ref / (100 * env.rounding_factor)) * env.rounding_factor;
      else
        return Math.round(discount_ref / env.rounding_factor) * env.rounding_factor;
    } catch (err) {
