...
class TicketAction extends Ticket {
  ...
  async cancelOrderLine (body, user) {
    // new funcitons creared

    if(body.hasOwnProperty('quantity') && body.quantity) {
      console.log('body', body);
      let _order_result = await models()['Order' + (this.test ? 'Test' : '')].aggregate([
        {
          $match: {
            "_id": mongoose.Types.ObjectId(body.orderId),
          },
        ...
        {
          $match: {
            "order_lines.product_instance_id": mongoose.Types.ObjectId(body.productIntanceId)
          },
        },
        {
          $project: {
        ...
        },
        {
          $group: {
            ...
            product_instance_id: {$first: '$product_instance_id'}
          }
        }
      ]).then(res => res[0]);
      // console.log('_order_result', _order_result);
      console.log(_order_result.order_lines);
      
      // let can_set_ticket = [];
      // _order_result.forEach(order => {
      //   order.order_lines.forEach(ol => {
      //     if (ol.tickets.length <= 1) {
      //       can_set_ticket.push({
      //         order_id: order._id,
      //         order_line_id: ol._id
      //       });
      //     }
      //   });
      // });
      // console.log('can_set_ticket', can_set_ticket);
