...
class TicketAction extends Ticket {
  ...
  async requestCancel(body, user) {
    try {
      ...
      if (!foundOrder.customer_id)
        throw new Error('guest orders cannot be canceled');
      ...
      if (body.orderLineId) {
        ...
        await order.changeStateAsCanceled(foundOrder, foundOrderLine);
        await new DSS(this.test).afterCancel(foundOrder, foundOrderLine)
        addedBalance = await order.calculateReturnPrice(foundOrder, foundOrderLine, true);

      } else {
        for (let i = 0; i < foundOrder.order_lines.length; i++) {
          if (!foundOrder.order_lines[i].cancel) {
            await order.changeStateAsCanceled(foundOrder, foundOrder.order_lines[i]);
            await new DSS(this.test).afterCancel(foundOrder, foundOrder.order_lines[i])
          }
        }

        addedBalance = await order.calculateReturnPrice(foundOrder, null, true);
      }
  ...
  async requestReturn(body, user) {
    try {
      ...
      if (body.orderLineId) {
        ...
        for (let i = 0; i < foundOrder.order_lines.length; i++) {
          if (foundOrder.order_lines[i].tickets.find(x => x.status !== _const.ORDER_LINE_STATUS.ReturnRequested) && !foundOrder.orderLines[i].cancel)
  ...
  async mismatchReport(trigger, user) {
    try {
      ...
      const DSS = require('./dss.model');
      // each order contains only one order line
      let orders;
      switch (trigger) {
  ...
  async damageInformed(orderId, orderLineId, user) {
    ...
    try {
      ...
      let DSS = require('./dss.model');
      return new DSS(this.test).afterDamageInformed(foundOrder, foundOrderLine, user);
