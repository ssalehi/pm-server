...
class TicketAction extends Ticket {
  ...
  async finalScan(orderId, barcode, user, isExternal, isCC) {
    try {
      ...
      const DSS = require('./dss.model');
      return new DSS(TicketAction.test).afterFinalCheck(foundOrder, foundProduct, user, isExternal, isCC);
    } catch (err) {
  ...
  async cancel(body, user) {
    try {
      ...
      if (!user.access_level) {
        if (order.customer_id.toString() !== user.id)
          throw new Error('only customer is able to cancel his/her order or order line')
      }

      let lastOrderTicket = order.tickets[order.tickets.length - 1];
      if (lastOrderTicket.status !== _const.ORDER_STATUS.WaitForAggregation)
        throw new Error('cancellation is allowed only for waiting for aggreation orders')

      let foundOrderLine;
      const Order = require('./order.model');
      const DSS = require('./dss.model');

      let changeOrdreLineState = async (order, orderLine) => {
        order = await new Order(this.test).changeStateAsCanceled(order, orderLine);
        orderLine = order.order_lines.find(x => x._id.toString() === orderLine._id.toString())
        return new DSS(TicketAction.test).afterCancel(order, orderLine, user);
      }
      ...
      if (body.orderLineId) {
        ...
        if (foundOrderLine.tickets.find(x => x.status === _const.ORDER_LINE_STATUS.Cancel))
        ...
        for (let i = 0; i < foundOrder.order_lines.length; i++) {
          await changeOrdreLineState(foundOrder, foundOrder.orderLines[i])
        }
