  ...
  async requestCancel(body, user) {
    try {
      ...
      if (body.orderLineId) {
        ...
        await changeOrdreLineState(foundOrder, foundOrderLine);

        addedBalance = await new OrderModel(this.test).calculateReturnPrice(foundOrder, foundOrderLine, true);
        ...
        addedBalance = await new OrderModel(this.test).calculateReturnPrice(foundOrder, null, true);
      }
