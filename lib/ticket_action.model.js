...
class TicketAction extends Ticket {
    ...
    try {
      ...
      switch (body.trigger) {
        ...
        case _const.SCAN_TRIGGER.SendInternal:
          return this.internalFinalScan(body.barcode, user);
        case _const.SCAN_TRIGGER.SendCustomer:
          return this.externalFinalScan(body.orderId, body.barcode, user);
  ...
  async internalFinalScan(barcode, user) {
    try {
      let foundProduct = await new ProductModel(this.test).getInstanceByBarcode(barcode);
      ...
      if (!foundOrder)
        throw error.orderNotFound;
      const DSS = require('./dss.model');
      return new DSS(TicketAction.test).afterFinalCheck(foundOrder, foundProduct, user);
    } catch (err) {
      console.log('-> error on internal final scan ', err);
      throw err;
    }
  }

  async externalFinalScan(orderId, barcode, user) {
    try {
      let foundProduct = await new ProductModel(this.test).getInstanceByBarcode(barcode);
      ...
      const DSS = require('./dss.model');
      return new DSS(TicketAction.test).afterFinalCheck(foundOrder, foundProduct, user, true);
    } catch (err) {
      console.log('-> error on internal final scan ', err);
      throw err;
    }
