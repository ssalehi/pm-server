...
class Delivery extends Base {
  ...
  initiate(order, order_line_id, from, to) {
    const fromCondition = from.customer._id ? {'from.customer.customer_id': {$eq: mongoose.Types.ObjectId(from.customer._id)}} : {'from.warehouse_id': {$eq: mongoose.Types.ObjectId(from.warehouse_id)}};
    const toCondition = to.customer._id ? {'to.customer.customer_id': {$eq: mongoose.Types.ObjectId(to.customer._id)}} : {'to.warehouse_id': {$eq: mongoose.Types.ObjectId(to.warehouse_id)}};

    return new Promise((resolve, reject) => {
      this.DeliveryModel.findOne({
        $and: [
