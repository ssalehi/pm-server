...
class Delivery extends DeliveryTicket {
  ...
  async initiate(order, orderLine, from, to, receiverId) {
    try {
      ...
      if ((from.customer && !from.customer.address_id) || (to.customer && !to.customer.address_id))
      ...
      const fromCondition = from.customer ?
        {'from.customer.address_id': {$eq: mongoose.Types.ObjectId(from.customer.address_id)}} :
        {'from.warehouse_id': {$eq: mongoose.Types.ObjectId(from.warehouse_id)}};

      const toCondition = to.customer ?
        {
          $and: [
            {'to.customer.address_id': {$eq: mongoose.Types.ObjectId(to.customer.address_id)}},
      ...
      if (preActiveDelivery && preActiveDelivery.length) { // existing active delivery
        ...
        this.makeNewDelivery(order, orderLine, startDay, from, to, isExternal, receiverId);
      }
  ...
  async requestPackage(deliveryId, user) {
    try {
      ...
      let isRetrun = !!(isExternal && delivery.from.customer);
      ...
      await new DSS(this.test).afterRequestForPackage(delivery, user.id, isExternal, isRetrun);
  ...
  async startDelivery(deliveryId, preCheck, user) {
    try {
      ...
      let matchedItems = await new DSS(this.test).afterDeliveryStarted(delivery, user, preCheck, isExternal, isRetrun);
  ...
  async endDelivery(deliveryId, user) {
    try {
      ...
      let isExternal = !!((delivery.from.customer && delivery.to.warehouse_id) || (delivery.to.customer && delivery.from.warehouse_id));
      let isReturn = !!(isExternal && delivery.from.customer);
      ...
      let DSS = require('./dss.model');
      return new DSS(this.test).afterDeliveryEnded(delivery, user, isExternal, isReturn);
