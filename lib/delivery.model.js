...
class Delivery extends Base {
  ...
  initiate(order, orderLineIds, from, to) {
    ...
    return this.DeliveryModel.findOne({
      ...
      .then(res => {
        if (res) {
          ...
          if (foundOrder)
            foundOrder.order_line_ids = foundOrder.order_line_ids.concat(orderLineIds.map(oli => mongoose.Types.ObjectId(oli)));
          else {
            res.order_details = res.order_details.concat([{
              order_id: mongoose.Types.ObjectId(order._id),
              order_line_ids: orderLineIds.map(oli => mongoose.Types.ObjectId(oli)),
            }]);
