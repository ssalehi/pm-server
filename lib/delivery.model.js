...
class Delivery extends DeliveryTicket {
  ...
  async requestPackage(deliveryId, user) {
    try {
      ...
      let isExternalRetrun = !!(isExternal && delivery.from.customer);
      ...
      await new DSS(this.test).afterRequestForPackage(delivery, user.id, isExternal, isExternalRetrun);
  ...
  async startDelivery(deliveryId, preCheck, user) {
    try {
      ...
      let matchedItems = await new DSS(this.test).afterDeliveryStarted(delivery, user, preCheck, isExternal, isExternalRetrun);
  ...
  async endDelivery(deliveryId, user) {
    try {
      ...
      let DSS = require('./dss.model');
      return new DSS(this.test).afterDeliveryEnded(delivery, user, isExternal, isExternalRetrun);
