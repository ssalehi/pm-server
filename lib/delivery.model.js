...
class Delivery extends DeliveryTicket {
  ...
  async initiate(order, orderLine, from, to, receiverId, isExternal = false) {
    try {
      ...
      if (deliveries && deliveries.length) { // previous delivery exists whether for current or noxt day
        ...
        if (deliveries.length === 2) // both current and next days deliveries are invalid
          ...
          this.makeNewDelivery(order, orderLine, nextDay, from, to, isExternal, receiverId);
