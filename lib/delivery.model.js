...
class Delivery extends Base {
    ...
    return this.DeliveryModel.aggregate([
      ...
      },
      {
        $project: {
          ...
          min_slot: 1,
          common_order_lines: {
            $setIntersection: ['$orders.order_lines._id', '$order_details.order_line_ids']
          }
        }
      },
      {
      ...
      {
        $addFields: {'cur_order_line': {$cmp: ['$orders.order_lines._id', '$common_order_lines']}}
      },
      {
        $match: {
          cur_order_line: 0,
      ...
      },
      {
        $lookup: {
          from: 'product',
          localField: 'orders.order_lines.product_id',
          foreignField: '_id',
          as: 'products'
        }
      },
      ...
      {
        $addFields: {'cur_product_instance': {$cmp: ['$products.instances._id', '$orders.order_lines.product_instance_id']}}
      },
      {
        $match: {
  ...
  assignDeliveryToAgnet(user, body) {
    ...
    // })
    // .then(res => {
    //   return models()['Order' + (Delivery.test ? 'Test' : '')].find({
    //     _id: {
    //       $in: deliveriesOrderDetails.map(el => el.order_id),
    //     },
    //     'order_lines._id': {
    //       $in: deliveriesOrderDetails.map(el => el.order_line_ids).reduce((a, b) => a.concat(b), [])
    //     }
    //   });
    // })
    // .then(res => {
    //   const ticket = new Ticket(Delivery.test);
