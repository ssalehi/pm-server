...
class Delivery extends Base {
  ...
  async initiate(order, orderLineId, from, to, isInternal = false) {
    ...
    if (delivery) {
      ...
      if (foundOrder) {
        let foundOrderLine = foundOrder.order_line_ids.find(x => x._id.toString() === orderLineId.toString());
        if (!foundOrderLine)
          foundOrder.order_line_ids.push(mongoose.Types.ObjectId(orderLineId));
      } else {
        delivery.order_details = delivery.order_details.concat([{
        ...
        if (orderEndDate) {
          if (moment(orderEndDate, 'YYYY-MM-DD').isBefore(moment(delivery.end, 'YYYY-MM-DD'))) {
            delivery.end = orderEndDate;
            delivery.slot = order.time_slot;
          } else if (moment(orderEndDate, 'YYYY-MM-DD').isSame(moment(delivery.end, 'YYYY-MM-DD'))) {
            // Calculate slot and end properties

            if (!delivery.slot && order.time_slot) {
              delivery.slot = {
                upper_bound: order.time_slot.upper_bound,
                lower_bound: order.time_slot.lower_bound,
              };
            } else if (delivery.slot && order.time_slot) {
              delivery.slot = {
                upper_bound: order.time_slot.upper_bound < delivery.slot.upper_bound ? order.time_slot.upper_bound : delivery.slot.upper_bound,
                lower_bound: order.time_slot.lower_bound < delivery.slot.lower_bound ? order.time_slot.lower_bound : delivery.slot.lower_bound,
              };
            }
          }
        }
      ...
      delivery.to = to;
      delivery.slot = order.time_slot;
      delivery.end = order.duration_days ? moment(order.order_time, 'YYYY-MM-DD').add(order.duration_days, 'days') : null;
      delivery.status_list = [];
    }
    ...
    return this.DeliveryModel.findOneAndUpdate(
      {
        ...
        $set: {
          ...
          status_list: delivery.status_list,
        }
      }, {
        new: true,
        upsert: true,
