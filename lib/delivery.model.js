...
class Delivery extends DeliveryTicket {
    ...
    try {
      ...
      let delivery = await (this.DeliveryModel.findOne({
        $and: [
          ...
          {$or: [{delivery_agent: {$exists: false}}, {delivery_agent: {$eq: null}}]},
          fromCondition,
          toCondition,
        ]
      }).lean());

      if (delivery) { // previous delivery exists either for current day or noxt day which has no agent
        ...
        currentDelivery = moment(delivery.start, 'YYYY-MM-DD').format('YYYY-MM-DD') === currentDay;
        nextDayDelivery = moment(delivery.start, 'YYYY-MM-DD').format('YYYY-MM-DD') === nextDayDelivery;
        ...
        if (foundOrder) {
          ...
          if (isExternal)
            throw errors.externalDeliveryInvalidOrder;
  ...
  async setDeliveryAgent(deliveryId, userId, agentId) {
    try {
      ...
      let delivery = await (this.DeliveryModel.findOneAndUpdate(
        ...
        {
          $set: {
            delivery_agent: mongoose.Types.ObjectId(agentId)
          }
        }, {
          new: true
        }
      ...
      await super.setDeliveryAsAgentSet(delivery, userId, agentId);

      if (!this.test)
        socket.sendToNS(userId);
   ...
   */
  async startDelivery(deliveryId, user, preCheck) {
    try {
