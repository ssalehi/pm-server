...
class Delivery extends DeliveryTicket {
  ...
  async initiate(order, orderLine, from, to, receiverId) {
    try {
      ...
      const isExternal = to.customer && to.customer._id;
      const isReturn = from.customer && from.customer._id;

      if ((isExternal && form.customer) || (isReturn && to.customer)) {
      ...
      if (deliveries && deliveries.length) { // previous delivery exists
        ...
        if (isExternal) {
          ...
          else { // there is only one delivery which is not open anymore
  ...
  async endDelivery(deliveryId, user) {
    try {
      ...
      await super.setDeliveryAsEnded(delivery, user.id)
