...
class Delivery extends Base {
  ...
  getDeliveryItems(user, offset, limit, body) {
    ...
    if (body.hasOwnProperty('isDelivered')) {
      if (body.isDelivered === false)
        isDeliveredMatching = {$or: [{'delivery_end': {$exists: false}}, {$and: [{'delivery_end': {$exists: true}, 'delivery_end': null}]}]}
  ...
  initiate(order, order_line_id, from, to) {
    const fromCondition = from.customer && from.customer.customer_id ? {'from.customer.customer_id': {$eq: mongoose.Types.ObjectId(from.customer.customer_id)}} : {'from.warehouse_id': {$eq: mongoose.Types.ObjectId(from.warehouse_id)}};
    const toCondition = to.customer && to.customer.customer_id ? {'to.customer.customer_id': {$eq: mongoose.Types.ObjectId(to.customer.customer_id)}} : {'to.warehouse_id': {$eq: mongoose.Types.ObjectId(to.warehouse_id)}};
    ...
    return this.DeliveryModel.findOne({
      ...
      .then(res => {
        if (res) {
          ...
          const foundOrder = res.order_details.find(o => mongoose.Types.ObjectId(o.order_id) === mongoose.Types.ObjectId(order._id));

          if (foundOrder)
            foundOrder.order_line_ids.push(order_line_id);
          else {
            res.order_details = res.order_details.concat([{
              order_id: mongoose.Types.ObjectId(order._id),
              order_line_ids: [mongoose.Types.ObjectId(order_line_id)]
            }]);

            const orderEndDate = moment(order.order_time, 'YYYY-MM-DD').add(order.duration_days, 'days');

            if (moment(res.min_end, 'YYYY-MM-DD').isBefore(moment(orderEndDate, 'YYYY-MM-DD'))) {
              res.min_end = orderEndDate;
              res.min_slot = order.time_slot;
            } else if (moment(res.min_end, 'YYYY-MM-DD').isSame(moment(orderEndDate, 'YYYY-MM-DD'))) {
