...
class Product extends Base {
      ...
      .then(res => {
        ...
        if (!body.id) {
          let newProduct = new this.ProductModel({
            ...
            brand: {name: brand.name, brand: type._id},
  ...
  setInventory(body) {
    ...
    if (!body.hasOwnProperty('price'))
      return Promise.reject(error.productInstancePriceRequired);
    ...
    return this.ProductModel.findById(mongoose.Types.ObjectId(body.id)).lean()
      .then(res => {
        ...
        let foundInstance = res.instances.find(x => x._id.toString() === body.productInstanceId);
        foundInstance.price = body.price;

        let foundInventory = foundInstance.inventory.find(i => i.warehouse_id.toString() === body.warehouseId);



        // verify order send a count = -1 to minus count by one and add to reserved
        if (Number.parseInt(body.count) !== -1 && Number.parseInt(body.count) < foundInventory.reserved)
          return Promise.reject(error.invalidInventoryCount);

        if (Number.parseInt(body.count) !== -1) {
          foundInventory.count = Number.parseInt(body.count);
        } else {

          foundInventory.count--;
          foundInventory.reserved++;

          if (foundInventory.reserved > foundInventory.count)
            return Promise.reject(error.invalidInventoryCount)
        }
  ...
  setImage(id, colorId, is_thumbnail, fileData) {
    ...
    return models['Color' + (Product.test ? 'Test' : '')].findById(mongoose.Types.ObjectId(colorId)).lean()
      ...
      .then(product => {
        ...
        if (res.n === 1) {
          return Promise.resolve({uploaded: fileData.filename});
        }
        else
