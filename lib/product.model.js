...
class Product extends Base {
  ...
  getAllProducts(){
    return this.ProductModel.find({}, 'name product_type brand base_price');
  }
  getProduct(id){
   ...
   *  images: list of images for a specific color
  ...
  setColor(body) {
    ...
    return this.ProductModel.findById(body.id).then(res => {
      ...
      // update current product_color
      if (body.productColorId) {// id of product color in colors array

        let product_color = res._doc.colors.id(body.productColorId);

        if (product_color) {
          product_color.color_id = body.colorId;
          product_color.images = body.images;
          return res.save();
        }
        else
        ...
        res._doc.colors.push({color_id: body.colorId, images: body.images});
        return res.save();
      }
    });
