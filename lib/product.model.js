...
class Product extends Base {
  ...
  getProducts(productIds, typeIds, tagIds, brandIds, isAdmin = false) {
    let queryArray = [];
                ...
                ,
                else:
                  {
                    $cond: [
                      {
                        '$and': [
                          {$gte: ['$campaign.end_date', new Date()]},
                          {$lte: ['$campaign.start_date', new Date()]},
                        ]
                      },
    ...
    if (isAdmin)
      postQuery.$group.campaigns = {
        $push: '$campaign'
      }

    query.push(postQuery);
  ...
  removeCampaign(productId, campaignId) {
  ...
  addCampaign(productId, campaignId) {
