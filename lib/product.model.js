...
class Product extends Base {
  ...
  setInventory(body) {
    ...
    return this.ProductModel.findById(mongoose.Types.ObjectId(body.id)).lean()
      ...
      .then(isSoldOut => {
        ...
        /**
         * if inventory is changed so that the instance is not sold out anymore, its flag should be removed
         * in opposite, if the instance has no inventory anymore it should be added to sold out collection but
         * its flag should not be changed immidately (because of 1 week off of sold out)
         */
  ...
  findOrderProductsInfo(productIds, productInsIds) {
    return this.ProductModel.aggregate([
      {
        $match: {_id: {$in: productIds}}
      }, {
      ...
      {
        $match: {
          'instances._id': {$in: productInsIds}
        }
      },
      {
        $group: {
      ...
      },
      {
        $project: {
          _id: 1,
          instances: 1,
          campaignInfo: {$max: '$campaignInfo.discount_ref'}
        }
      }
    ])
  }
