...
class Product extends Base {
  ...
  getProduct(id) {
    ...
    return this.getProducts([id]).then(r => Promise.resolve(r[0]));
  }
  ...
  getProducts(productIds, typeIds, tagIds, brandIds) {
    let queryArray = [];
    ...
    return this.ProductModel.aggregate(query);
  }
  ...
  setInventory(body) {
    ...
    return this.ProductModel.findById(mongoose.Types.ObjectId(body.id)).lean()
      .then(res => {
        ...
        let initialCount = foundInventory.count;
        ...
        return this.checkSoldOutStatus(body.id, body.productInstanceId, initialCount, foundInventory.count);
      })
      .then(res => {
        return this.ProductModel.update({
          _id: mongoose.Types.ObjectId(body.id),
  ...
  checkSoldOutStatus(productId, productInstanceId, initialCount, changedCount) {

    if (changedCount === 0) {
      return new SoldOutModel(Product.test).insertProductInstance(productId, productInstanceId);
    }
    else if (initialCount === 0 && changedCount > 0) {
      return new SoldOutModel(Product.test).removeProductInstance(productId, productInstanceId);
    }
