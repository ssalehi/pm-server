...
class Product extends Base {
  ...
  getProducts(productIds, typeIds, tagIds, getInstances = true) {
    let queryArray = [];
  ...
  setInventory(body) {
    ...
    return this.ProductModel.findById(mongoose.Types.ObjectId(body.id)).then(res => {
      ...
      let inventory = product_instance._doc.inventory.find(i => i._doc.warehouse_id.toString() === body.warehouseId);

      if (inventory) { // update current existing inventory
        inventory.count = (body.count !== -1 && !body.hasOwnProperty('incCount')) ? body.count : (inventory.count + body.incCount);
        inventory.reserved = body.reserved ? body.reserved : (inventory.reserved + body.incReserved );
