...
class Product extends Base {
  ...
  getProductByColor(product_id, color_id) {
    ...
    return this.ProductModel.aggregate([
      ...
      {
        $match: {'instances.product_color_id': mongoose.Types.ObjectId(color_id)}
      },
      {
        $unwind: {
          path: '$colors',
    ...
    return this.ProductModel.findOne({
      "_id": mongoose.Types.ObjectId(id),
        ...
        res.colors.forEach((color, index) => {
          if (color.color_id.equals(colorId)) {
            found = true;
            if (!is_thumbnail) {
              res.colors[index]['image']['angles'].push(path);
            }
            else {
              res.colors[index]['image']['thumbnail'] = path;
            }
            return res.save((err) => {
              if (err) throw err;
        ...
        if (!found) {
          let newColor = {
            color_id: colorId,
            image: {
              angles: is_thumbnail ? [] : [path],
              thumbnail: is_thumbnail ? path : ''
            }
          };
      ...
      if (!res)
        return Promise.resolve({downloadURL: path});
