...
class Product extends Base {
  ...
  setInventory(body) {
    ...
    return this.ProductModel.findById(mongoose.Types.ObjectId(body.id)).then(res => {
      ...
      let product_instance = res._doc.instances.find(r => r._doc._id.toString() === body.productInstanceId);

      if (body.price) {
        product_instance.price = body.price;
      }

      let inventory = product_instance.inventory.find(i => i._doc.warehouse_id.toString() === body.warehouseId);

      if (inventory) { // update current existing inventory
        inventory.count = body.count >= 0 && !body.incCount ? body.count : (inventory.count + body.incCount);
        inventory.reserved = body.reserved >= 0 ? body.reserved : (inventory.reserved + (body.incReserved ? body.incReserved : 0));

      } else {
        product_instance._doc.inventory.push({
          warehouse_id: mongoose.Types.ObjectId(body.warehouseId),
          count: body.count,
          reserved: body.reserved ? body.reserved : 0
        });
      }
