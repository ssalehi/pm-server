...
class Ticket {
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.type === 'inbox') {
      ...
      if(user.access_level === _const.ACCESS_LEVEL.SalesManager) 
        search = this.searchTrackingOrders(options, offset, limit);
  ...
  searchTrackingOrders(body ,offset, limit) {
    console.log('searchTrackingOrders');
    ...
    if (body.hasOwnProperty('invoiceNo') && body.invoiceNo) {
      conditonInvoiceNo = {'invoice_no':  new RegExp('.*' + body.invoiceNo.toLowerCase() + '.*')}
    }
    // transferee 
    if (body.hasOwnProperty('transferee') && body.transferee) {
      conditonTransferee.$or.push(
        {'customer.name':  new RegExp('.*' + body.transferee.toLowerCase() + '.*')},
        {'customer.surname':  new RegExp('.*' + body.transferee.toLowerCase() + '.*')}
      )
    ...
    if (body.hasOwnProperty('status') && body.status) {
      conditonStatus = {'order_lines.tickets.status' : body.status}
    }
  ...
  searchInbox(receiverId, offset, limit, accessLevel) {
    ...
    // check sales manager access delete receiver_id
    if(accessLevel === _const.ACCESS_LEVEL.SalesManager) {
