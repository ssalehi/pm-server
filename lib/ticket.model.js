...
class Ticket {
  ...
  closeCurrentTicket(order, orderLine, userId, isOrderTicket = false) {
    ...
    if (!isOrderTicket)
      ...
      return new Order(this.test).model.update(query, {
        '$set': {
          'tickets.$[j].is_processed': true,
          'tickets.$[j].agent_id': userId,
        }
      }, {
          arrayFilters: [
            {
  ...
  setTicket(order, orderLine, status, userId, receiverId, desc = null, isLastTicket = false, isOrderTicket = false) {
    ...
    return this.closeCurrentTicket(order, orderLine, userId, isOrderTicket)
      .then(res => this.addNewTicket(order, orderLine, status, receiverId, null, isLastTicket, userId, isOrderTicket))
      .then(res => {
        order = res;
        if (isOrderTicket)
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.type === 'inbox') {
      ...
      if (user.access_level === _const.ACCESS_LEVEL.ShopClerk)
        search = this.searchInbox(user.warehouse_id, offset, limit, !!options.manual);
    }
  ...
  searchInbox(receiverId, offset, limit, manual) {
    return manual ? this.manualInbox(receiverId, offset, limit) : this.scanInbox(receiverId, offset, limit);
  }

  scanInbox(receiverId, offset, limit) {
    const result = {
      mainQuery: [],
      countQuery: []
    };
  ...
  manualInbox(receiverId, offset, limit) {
    const result = {
      mainQuery: [],
      countQuery: []
    };
    ...
    if (body.hasOwnProperty('last_ticket') && body.last_ticket) {
      optimizeQuery = {
        $match: {
          $and: [
            ...
            {"last_ticket.status": _const.ORDER_LINE_STATUS.ReadyToDeliver},
          ]
        }
      };
    } else {
      optimizeQuery = {
        $match: {
          ...
          $and: [
            {'order_lines.tickets.receiver_id': mongoose.Types.ObjectId(receiverId)},
            {"last_ticket.status": {$ne: _const.ORDER_LINE_STATUS.ReadyToDeliver}},
            {"order_lines.tickets.status": _const.ORDER_LINE_STATUS.ReadyToDeliver},
          ]
        }
      };
    }
