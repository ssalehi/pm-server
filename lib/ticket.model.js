...
class Ticket {
  ...
  searchOutbox(receiverId, offset, limit) {
    ...
    result.mainQuery = [
      ...
      {
        $match: {
          'order_lines.tickets.receiver_id': {$eq: mongoose.Types.ObjectId(receiverId)},
          $or: [
            {"order_lines.tickets.status":  _const.ORDER_STATUS.ReadyToDeliver},
            {"order_lines.tickets.status":   _const.ORDER_STATUS.DeliverySet},
            {"order_lines.tickets.status":   _const.ORDER_STATUS.OnDelivery},
      ...
      },
      {
        $project: {
          ...
          check_field: {
            $cond:[{
              "$or": [
                {"$eq": ["$last_ticket.status", _const.ORDER_STATUS.ReadyToDeliver]},
                {"$eq": ["$last_ticket.status", _const.ORDER_STATUS.DeliverySet]},
                {"$eq": ["$last_ticket.status", _const.ORDER_STATUS.OnDelivery]},
                {"$eq": ["$last_ticket.status", _const.ORDER_STATUS.Delivered]}
              ]},1,0]
          },
          last_ticket: 1,
          used_balance: 1,
