...
class Ticket {
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.type === 'inbox') {
      ...
      if (user.access_level === _const.ACCESS_LEVEL.ShopClerk)
        search = this.searchShopInbox(user.warehouse_id, offset, limit, !!options.isCollect);
    }
  ...
  searchShopInbox(receiverId, offset, limit, is_collect) {
    ...
    let preMain = [
      ...
      },
      {
        $project: {
          ...
          is_collect: 1,
          order_time: 1,
          customer_id: is_collect ? 1 : -1,
          order_line_id: '$order_lines._id',
          adding_time: '$order_lines.adding_time',
          tickets: '$order_lines.tickets',
          product_colors: '$product.colors',
          total_order_lines: '$total_order_lines',
          address: is_collect ? 1 : -1,
          instance: {
            '_id': '$product.instances._id',
            'product_id': '$product._id',
            'product_name': '$product.name',
    ...
    let mainGroupByOrder = !is_collect ? [] :
      [
        {
          $lookup: {
            from: 'customer',
    ...
    result.mainQuery = [...preMain, ...mainGroupByOrder, ...postMain];
    ...
    let countGroupByOrder = !is_collect ? [] :
      [
        {
          $group: {
            _id: '$_id'
    ...
    result.countQuery = [...preCount, ...countGroupByOrder, ...postCount];
    return result;
  }
