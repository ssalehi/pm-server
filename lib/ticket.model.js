...
class Ticket {
  ...
  search(options, offset, limit, user) {
    ...
    if(options && options.type === 'readyToDeliver') {
    if(!user.warehouse_id)
      return Promise.reject(error.ticketReceiverRequired)

      search = this.searchReadyToDeliver(user.warehouse_id, offset, limit);
    }
  ...
  searchReadyToDeliver(receiverId, offset, limit) {

    console.log('----vcv>', receiverId);
    const result = {
      mainQuery: [],
      countQuery: []
    };
