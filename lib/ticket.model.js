...
class Ticket extends Base {
  ...
  async setOrderLineAsCanceled(order, orderLine, userId, warehouseId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.Cancel, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as cancel ', err);
      throw err;
    }
  }
  ...
  async setOrderAsCanceled(order, userId) {
    try {



    } catch (err) {
      console.log('-> error on set order as canceled', err);
      throw err;
    }
  ...
  async search(options, offset, limit, user) {
    ...
    try {
      ...
      if ([_const.ACCESS_LEVEL.ShopClerk, _const.ACCESS_LEVEL.HubClerk].includes(user.access_level)) { // shop of hub views
  ...
  getDeliveryAgentOrderLines(receiverId, isExternal = false) {
    return this.OrderModel.aggregate([
      ...
      },
      {
        $match: {
          ...
          {
            'last_ticket.status': {
              $eq: isExternal ? _const.ORDER_LINE_STATUS.Checked : _const.ORDER_LINE_STATUS.ReadyToDeliver
            }
          }]
        }
      },
