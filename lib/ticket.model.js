...
class Ticket extends Base {
  ...
  async setOrderLineAsWatingForLostWarehouse(order, orderLine, userId, warehouseId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.WaitForLostWarehouse, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as waiting for online warehouse');
      throw err;
    }
  ...
  async setOrderLineAsWaitForDamage(order, orderLine, userId, salesManagerId) {
    try {

      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.WaitForDamage, userId, salesManagerId);
    } catch (err) {
      console.log('-> error on set order line as cancel requested ', err);
      throw err;
    }
