...
class Ticket extends Base {
  ...
  async setOrderLineAsWatingForOnlineWarehouseCancel(order, orderLine, userId, warehouseId) {
    try {
      ...
      console.log('-> error on set order line as waiting for online warehouse cancel');
      throw err;
    }
  }
  
  async setOrderLineAsWatingForReturn(order, orderLine, userId, warehouseId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.WaitForReturn, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as waiting for online warehouse cancel');
      throw err;
    }
  ...
  async setOrderLineAsReturnVerified(order, orderLine, userId, warehouseId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.ReturnVerified, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as waiting for online warehouse cancel');
      throw err;
    }
  ...
  async setOrderLineAsDamageWithRefund(order, orderLine, userId, warehouseId) {
    try {

      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.DamageWithRefund, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as wait for damage with refund ', err);
      throw err;
    }
  }
  
  async setOrderLineAsDamageWithoutRefund(order, orderLine, userId, warehouseId) {
    try {

      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.DamageWithoutRefund, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as wait for damage without refund ', err);
      throw err;
    }
  }

  async setOrderLineAsWaitingForLost(order, orderLine, userId, warehouseId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.WaitForLost, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as waiting for lost warehouse');
      throw err;
    }
  }
  ...
  async setOrderLineAsVerfiedByOnlineWarhouse(order, orderLine, userId, warehouseId) {
    try {
      ...
      console.log('-> error on set order line as verified by online warehouse');
      throw err;
    }
  }

  async setOrderLineAsDamageWithRefundVerified(order, orderLine, userId, salesManagerId) {
    try {

      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.DamageWithRefundVerified, userId, salesManagerId);
    } catch (err) {
      console.log('-> error on set order line as damage with refund verified ', err);
      throw err;
    }
  }
  
  async setOrderLineAsDamageWithoutRefundVerified(order, orderLine, userId, salesManagerId) {
    try {

      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.DamageWithoutRefundVerified, userId, salesManagerId);
    } catch (err) {
      console.log('-> error on set order line as damage without refund verified ', err);
      throw err;
    }
  }
  
  async setOrderLineAsLostVerified(order, orderLine, userId, warehouesId) {
    try {

      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.LostVerified, userId, warehouesId);
    } catch (err) {
      console.log('-> error on set order line as lost verified ', err);
      throw err;
    }
  }
  ...
  async setOrderLineAsDeliverySet(order, orderLine, userId, recieverId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.DeliverySet, userId, recieverId);
    } catch (err) {
  ...
  async setOrderLineAsReturnRequested(order, orderLine, customerId, salesManagerId) {
    try {
      ...
      console.log('-> error on set order line as return requested ', err);
      throw err;
    }
  }
  ...
  async setOrderLineAsReserved(order, order_line, warehouse, renew = false) {
    ...
    try {
      let res = await new ProductModel(this.test).setInventory(order_line.product_id, order_line.product_instance_id, warehouse._id, null, null, 1, null);
    ...
    try {
      ...
      if ([_const.ACCESS_LEVEL.ShopClerk, _const.ACCESS_LEVEL.HubClerk].includes(user.access_level)) { // shop or hub views
        ...
        if (options && options.type === _const.LOGISTIC_SEARCH.ScanReturnDelivery) {
          ...
          search = this.searchScanInternalReturnBox(user.warehouse_id, offset, limit);
        }
      }
          ...
          },
          {
            'last_ticket.status': {
              $in: [
                ...
                _const.ORDER_LINE_STATUS.DamageWithRefund,
                _const.ORDER_LINE_STATUS.DamageWithoutRefund,
      ...
      },
      {
        $match: {
          $and: [
            ...
            {'order_lines.tickets.status': {$ne: _const.ORDER_LINE_STATUS.ReturnRequested}}
          ]
        }
      },
      {
  ...
  searchScanInternalReturnBox(receiverId, offset, limit) {
        ...
        $match: {
          ...
          },
          {
            $or: [
              ...
              {'order_lines.tickets.status': {$eq: _const.ORDER_LINE_STATUS.ReturnRequested}}
            ]
          },
          {
            'last_ticket.receiver_id': {
  ...
  getDeliveryAgentOrderLines(receiverId, isExternal = false, isExternalReturn = false, isInternalReturn = false) {
    ...
    try {
      ...
      console.log('-> error on get delivery agent order lines', err);
      throw err;
    }
