...
class Ticket {
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.type === 'inbox') {
      if (!user.warehouse_id)
        return Promise.reject(error.ticketReceiverRequired);
      // added options to arguments
      search = this.searchInbox(user.warehouse_id, offset, limit, options);
    }
  ...
  searchInbox(receiverId, offset, limit, body) {
    // added new feature with body 
