...
class Ticket extends Base {
  ...
  async setOrderLineAsFinalCheck(order, orderLine, userId, receiverId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.FinalCheck, userId, warehouseId);
    } catch (err) {
      console.log('-> error on set order line as final check ', err);
  ...
  async setOrderLineAsReadyToDeliver(order, orderLine, userId, receiverId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.FinalCheck, userId, receiverId);
    } catch (err) {
      console.log('-> error on set order line as final check ', err);
      throw err;
    }
  ...
  getDeliveryAgentOrderLines(userId) {
    return this.OrderModel.aggregate([
      {
        $match: {
          $and: [{
