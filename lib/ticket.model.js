...
class Ticket {
   ...
   * 
   */
  addNewTicket(order, orderLine, status, receiverId, desc = null, userId = null, isOrderTicket = false) {
  ...
  setTicket(order, orderLine, status, userId, receiverId, desc = null, isOrderTicket = false) {
    ...
    return this.closeCurrentTicket(order, orderLine, userId, isOrderTicket)
      .then(res => this.addNewTicket(order, orderLine, status, receiverId, desc, userId, isOrderTicket))
      .then(res => {
        order = res;
        if (!isOrderTicket)
  ...
  async setOrderLineAsVerfiedByOnlineWarhouse(order, orderLine, userId, warehouseId) {
    try {
      return this.setTicket(order, orderLine, _const.ORDER_LINE_STATUS.OnlineWarehouseVerified, userId, warehouseId, null);
    } catch (err) {
      console.log('-> error on set order line as waiting for online warehouse');
      throw err;
    }
  ...
  searchSendToCustomerBox(receiverId, offset, limit) {
    ...
    result.mainQuery = [
      ...
      },
      {
        $match: {
          $and: [{
            'last_order_ticket.is_processed': false
          },
          {
            'last_order_ticket.receiver_id': {
              $eq: mongoose.Types.ObjectId(receiverId)
            }
          },
          {
            'last_order_ticket.status': {
