...
class Ticket extends Base {
  ...
  async setOrderAsReadyForInvoice(order, userId, warehouseId) {
    try {
      ...
      this.setTicket(order, null, _const.ORDER_STATUS.ReadyForInvoice, userId, warehouseId, null, true);
  ...
  async search(options, offset, limit, user) {
    ...
    try {
      ...
      if (options && options.type === _const.LOGISTIC_SEARCH.ScanInternalDelivery) {
        search = this.searchScanInternalDeliveryBox(user.warehouse_id, offset, limit);
      }
      if (options && options.type === _const.LOGISTIC_SEARCH.ScanExternalDelivery) {
        search = this.searchScanExternalDeliveryBox(user.warehouse_id, offset, limit);
      }
  ...
  searchScanInternalDeliveryBox(receiverId, offset, limit) {
    const result = {
      mainQuery: [],
      countQuery: []
    };
  ...
  searchScanExternalDeliveryBox(receiverId, offset, limit) {
    const result = {
      mainQuery: [],
      countQuery: []
    };
  ...
  shelvesList(receiverId, offset, limit, body) {
    ...
    if (body.hasOwnProperty('trackingCode') && body.trackingCode) {
      nameMatching.push({'transaction_id': new RegExp('.*' + body.trackingCode + '.*')});
    }
    ...
    result.mainQuery = [
      ...
      },
      {
        $project: {
          ...
          customer_name: {$toLower: {$concat: ['$customer.first_name', ' ', '$customer.surname']}},
          recipient_name: {$toLower: {$concat: ['$recipient_name', ' ', '$recipient_surname']}},
          cmp_value: {
            $cmp: ['$order_lines.product_instance_id', '$product.instances._id']
          }
        },
    ...
    console.log('result main query', result.mainQuery);
    return result;
  }
