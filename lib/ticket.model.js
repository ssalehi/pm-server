...
class Ticket {
   ...
   */
  addNewTicket(order, orderLine, status, receiverId, desc = null) {
  ...
  setTicket(order, orderLine, status, userId, receiverId) {
    ...
    if (orderLine.tickets.find(x => !x.is_processed && x.status === status))
    ...
    return this.closeCurrentTicket(order, orderLine, userId)
      .then(res => this.addNewTicket(order, orderLine, status, receiverId))
  }
  ...
  search(options, offset, limit, user) {
    ...
    if (options && options.type === 'outbox') {
      ...
      search = this.searchOutbox(user.warehouse_id, offset, limit);
    }
  ...
  searchInbox(receiverId, offset, limit) {
    ...
    result.mainQuery = [
      ...
      {
        $addFields: {'total_order_lines': {$size: '$order_lines'}}
      },
      {
        $unwind: {
          path: '$customer',
...

      {
        $match: {
          $and: [
            ...
            {'last_ticket.receiver_id': {$eq: mongoose.Types.ObjectId(receiverId)}},
            {
              'last_ticket.status': {
                $nin: [
  ...
  searchOutbox(receiverId, offset, limit) {
    ...
    result.mainQuery = [
      ...
      {
        $addFields: {'last_ticket': {"$arrayElemAt": ["$order_line.tickets", -1]}}
      },
      {
        $unwind: {
      ...
      },
      {
        $match: {
          ...
          'order_line.tickets.receiver_id': {$eq: mongoose.Types.ObjectId(receiverId)},
          $or: [
            {"order_line.tickets.status": _const.ORDER_STATUS.ReadyToDeliver},
            {"order_line.tickets.status": _const.ORDER_STATUS.DeliverySet},
            {"order_line.tickets.status": _const.ORDER_STATUS.OnDelivery},
            {"order_line.tickets.status": _const.ORDER_STATUS.Delivered}
          ]
        }
      },
      ...
      },
      {
        $group: {
          _id: '$_id',
          customer: {$first: '$customer'},
          last_ticket: {$first: '$last_ticket.status'},
